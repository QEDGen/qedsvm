...
fn lift_one(
    ...
    // H8 (large-binary decode pin): the full `decodeProgram` bridge was
    // omitted above, so pin the byte→Insn decode of every walked PC
    // directly against the real `.text` bytes. The bytes are embedded
    // as a hex STRING (one token — a `#[..]` ByteArray literal of this
    // size blows `maxRecDepth`), the slot map is computed in-kernel by
    // `Decode.buildSlotMap`, and a single `native_decide` compares
    // `decodeInsn` at each walked byte offset against the exact
    // instruction term the CodeReq above uses. Internal `call_local`
    // PCs are excluded: their imm in a relocated binary is a murmur3
    // hash `decodeInsn` cannot resolve until the H2 hash→PC registry
    // lands; their targets come from solana-sbpf's function registry
    // in the emitter (which errors loudly on an unresolved call).
    if !emit_decode_bridge && !block_pcs.is_empty() {
        let mut pin_offs: Vec<String> = Vec::new();
        let mut pin_exps: Vec<String> = Vec::new();
        let mut skipped_call_locals: Vec<usize> = Vec::new();
        for &pc in &block_pcs {
            let lean_insn = if let Some(ctor) = state.syscall_pcs.get(&pc) {
                format!(".call {}", ctor)
            } else if insns[pc].opc == 0x85 {
                skipped_call_locals.push(pc);
                continue;
            } else {
                let tgt = resolve_call_target_logical(ctx, &analysis, &insns[pc]);
                let jtgt = Some(resolve_jump_target(ctx, pc, insns[pc].off as i64));
        ...
        let skip_note = if skipped_call_locals.is_empty() {
            String::new()
        } else {
            format!(
                "\n    Internal `call_local` PCs {:?} are NOT pinned: their \
                 relocated imm\n    is a murmur3 hash of the target \
                 (soundness-audit H2); the emitter\n    resolves them via \
                 solana-sbpf's function registry instead.",
                skipped_call_locals)
        };
        out.push_str(&format!(
            "/-- Walked-PC decode pins (soundness-audit H8): every \
             instruction the\n    Hoare triple's `CodeReq` claims at a \
             walked PC is the decode of the\n    `.text` bytes at that \
             PC's byte offset, checked end-to-end in-kernel\n    \
             (`bytesOfHex` → `buildSlotMap` → `decodeInsn`) by \
             `native_decide`.{} -/\n",
            skip_note));
