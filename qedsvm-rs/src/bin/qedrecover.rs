...
/// Search the instruction stream from `start_pc` for the dispatcher
/// pattern: a `ldxX rD, [rS + 0]` (discriminator load) immediately
/// followed by a `jeqX rD, imm=disc_value, +off` (match-and-branch).
///
/// Returns `(load_pc, jeq_pc, target_pc)` for the first match.
///
/// Limitation: assumes the dispatcher is a flat sequence of
/// `ldx + jeq imm` pairs (the pinocchio pattern). Jump tables and
/// other compaction schemes aren't handled — would need framework-
/// specific knowledge.
fn find_dispatch_arm(
    ...
    while pc + 1 < n {
        ...
        if load.opc == load_opc && load.off == 0 {
            let jeq = &instructions[pc + 1];
            let is_jeq_imm = jeq.opc == ebpf::JEQ32_IMM
                          || jeq.opc == ebpf::JEQ64_IMM;
            if is_jeq_imm && jeq.imm == disc_value && jeq.dst == load.dst {
                // `jeq` branch target is encoded as a signed offset
                // relative to the instruction *after* the jeq.
                let target = (pc as i64) + 1 + 1 + (jeq.off as i64);
                return Some((pc, pc + 1, target as usize));
            }
        }
        pc += 1;
    }
...
fn emit_lean<W: std::io::Write>(
    ...
    writeln!(out, "def refinesIntrinsic : String := \"{}\"", ovix.refines)?;
    writeln!(out)?;
    writeln!(out, "/-- CU budget claimed by the overlay. -/")?;
    writeln!(out, "def cuBudget : Nat := {}", ovix.cu_budget)?;
    writeln!(out)?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== overlay scope ===");
    for ovix in &overlay.instructions {
        // Look up matching IDL instruction.
