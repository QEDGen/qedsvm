...
use qed_analysis::{image::ProgramImage, layout::codama_number_size};
...
/// Run the qedrecover command using process arguments.
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let image = ProgramImage::load(&args.so)?;
    let analysis = Analysis::from_executable(&image.executable)?;
    let pc_map = &image.pc_map;
        ...
        match recover_one(&analysis, pc_map, idl_ix)? {
            ...
            Recovery::Arm(recovered) => {
                ...
                if let Some(meta_path) = &args.qedmeta_out {
                    ...
                    let so_sha256 = hex(&sha2::Sha256::digest(&image.elf_bytes));
...
mod tests {
    ...
    fn qedmeta_sidecar_emits_recovered_facts() {
        ...
        // Parse with the exact shared type qedlift consumes.
        let meta: qed_artifacts::QedMeta =
            toml::from_str(&emitted).expect("emitted sidecar must parse");
        ...
        let rec = meta.instructions[0]
            .recovered
            .as_ref()
            .expect("recovered facts emitted");
