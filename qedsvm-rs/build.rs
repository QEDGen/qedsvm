...
fn main() {
    ...
    if !lake_lean_dir.exists() {
        panic!(
            "Lake build output missing at {}.\n\
             Run `lake build` in the workspace root before building qedsvm-rs.",
    ...
    // ─ Every qedsvm module dylib produced by Lake. ─────────────
    //
    // Lake emits them with the module-mangled name and no `lib`
    // prefix (e.g. `qedsvm_Svm_Ffi.dylib`), so cargo's normal
    ...
    //
    // The Lean dylibs (`qedsvm_Svm_SBPF_Curve25519.dylib`, etc.)
