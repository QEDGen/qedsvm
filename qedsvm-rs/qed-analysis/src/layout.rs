...
fn codama_type_size(
    ...
    match ty.get("kind")?.as_str()? {
        ...
        "optionTypeNode" => {
            let prefix = ty
                .get("prefix")
                .and_then(|p| codama_type_size(p, defined))
                .unwrap_or(4);
            Some(prefix + codama_type_size(ty.get("item")?, defined)?)
        }
        "booleanTypeNode" => ty
            .get("size")
            .and_then(|s| codama_type_size(s, defined))
            .or(Some(1)),
        "enumTypeNode" => ty
            .get("size")
            .and_then(|s| codama_type_size(s, defined))
...
mod tests {
    ...
    fn token_account_layout() {
        ...
        assert_eq!(
            (by("mint").offset, by("mint").kind.clone()),
            (0, FieldKind::Pubkey)
        );
        assert_eq!(
            (by("owner").offset, by("owner").kind.clone()),
            (32, FieldKind::Pubkey)
        );
        assert_eq!(
            (by("amount").offset, by("amount").kind.clone()),
            (64, FieldKind::U64)
        );
        // delegate (option<pubkey>) starts the opaque rest region at 72.
        assert_eq!(by("delegate").offset, 72);
        assert_eq!(
            (by("state").offset, by("state").kind.clone()),
            (108, FieldKind::Byte)
        );
    }
