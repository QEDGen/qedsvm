...
/// Core VM conformance: interpreter semantics, metering, loading,
/// account write-back — the plain (non-syscall, non-CPI) fixtures.
mod core_vm {
    use super::*;
...
/// Happy-path syscall fixtures: memory ops, hashing, PDA, return
/// data, sysvars, compute meter.
mod syscalls {
    use super::*;
...
/// Typed VM-fault conformance (audit H6 et al.): OOB syscall
/// inputs/outputs and the abort syscall must fault on BOTH engines.
mod oob {
    use super::*;
...
/// Canonical SPL Token program (real on-chain binary).
mod spl_token {
    use super::*;
...
/// p-token (pinocchio SPL Token reimplementation): happy paths +
/// the Layer-3 guard fan-out (violating fixtures) + the two pinned
/// wraps_on_both NON-guard findings.
mod p_token {
    use super::*;
...
/// Cross-program invocation: BPF-to-BPF calls, write-back, realloc,
/// readonly protection, PDA signer promotion, return data, depth-2 chains.
mod cpi {
    use super::*;
...
/// CPI into native programs: System program variants,
/// ComputeBudget, and the devnet Janus reproducer.
mod system_cpi {
    use super::*;
