...
## `sbpfv3_static_path.so` / `sbpfv3_account_path.so`

Generated by `python3 build_sbpfv3_static_path.py` in this directory. The
sectionless V3 ELF has a JMP32 branch, relative internal call, and static
`sol_log_64_` syscall. `RunnerTests` embeds the same instructions for a
Lean-checked exit theorem; `diff_mollusk.rs` compares its runtime behavior.
The account-path variant also increments the first byte of account 0's data
at serialized-input offset 96. Its pinned Lean theorem checks the byte
update, and its differential test compares the account data and 110 CU.
`qedlift` generates `Generated.Sbpfv3AccountPathLifted` from this binary and
`sbpfv3_account_path.pcs`. SHA-256 values:
