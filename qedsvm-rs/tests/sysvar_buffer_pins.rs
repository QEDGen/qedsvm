...
fn dump_sysvar_buffers() {
    ...
    let entries: Vec<(&str, [u8; 32], Vec<u8>)> = vec![
        (
            "clock",
            solana_sdk_ids::sysvar::clock::id().to_bytes(),
            bincode::serialize(&s.clock).unwrap(),
        ),
        (
            "epoch_schedule",
            solana_sdk_ids::sysvar::epoch_schedule::id().to_bytes(),
            bincode::serialize(&s.epoch_schedule).unwrap(),
        ),
        (
            "epoch_rewards",
            solana_sdk_ids::sysvar::epoch_rewards::id().to_bytes(),
            bincode::serialize(&s.epoch_rewards).unwrap(),
        ),
        (
            "rent",
            solana_sdk_ids::sysvar::rent::id().to_bytes(),
            bincode::serialize(&s.rent).unwrap(),
        ),
        (
            "slot_hashes",
            solana_sdk_ids::sysvar::slot_hashes::id().to_bytes(),
            bincode::serialize(&s.slot_hashes).unwrap(),
        ),
        (
            "stake_history",
            solana_sdk_ids::sysvar::stake_history::id().to_bytes(),
            bincode::serialize(&s.stake_history).unwrap(),
        ),
        (
            "last_restart_slot",
            solana_sdk_ids::sysvar::last_restart_slot::id().to_bytes(),
...
fn sysvar_buffers_match_lean_constants() {
    ...
    let pin_id = |name: &str, actual: [u8; 32], head: [u8; 8]| {
        // pin first 8 bytes of each sysvar id
