/-
  ...
  crates via `rust-bridge/` (declared as `@[extern] opaque` on the Lean
  ...
  When agave bumps a pin, the matching `rust-bridge/Cargo.toml` line
...
`Svm/Syscalls/Sha512.lean`, and implemented in `rust-bridge` by a
...
via `rust-bridge`. Agave's `solana-keccak-hasher` wraps the same
...
Both bridge to `curve25519-dalek` via `rust-bridge`, exactly as
