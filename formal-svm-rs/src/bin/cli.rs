//! `formal-svm-cli` — drive the Lean reference SVM from the shell.
//!
//! ```
//! formal-svm-cli --elf prog.so --input input.bin [--cu-budget 200000]
...
use formal_svm::{run_buffer, run_buffer_with_registry, ExitOutcome};

const USAGE: &str = "\
formal-svm-cli — execute a Solana ELF under the formal-svm Lean reference VM.

USAGE:
  formal-svm-cli --elf <path> --input <path> [OPTIONS]
...
OPTIONS:
  ...
  --registry <path>  CPI program registry blob (see
                     `formal_svm::encode_registry`). Without this,
