...
//!   (`initialize_formalSvm_Svm_Ffi`) and our `@[export]`'d entry
//!   point `formal_svm_run_elf_buffer`. Both come from
//!   `formalSvm_Svm_Ffi.dylib`.
...
//! `formal_svm_run_elf_buffer` call, and the post-call ByteArray
...
pub fn lock() -> LeanGuard<'static> {
    ...
    if !*guard {
        ...
        unsafe {
            ...
            let res = initialize_formalSvm_Svm_Ffi(/* builtin = */ 1);
