...
set_option maxHeartbeats 1600000 in
/-- Single-seed `sol_create_program_address` success triple. Off-curve is the
    opaque-FFI hypothesis `hOffCurve`; the derived PDA is the pure
    `Sha256.hash (seedBytes ++ pidBytes ++ PDA_MARKER)`. Modelled on
    `call_sol_sha256_spec` (+1 register r4, +1 read region program_id). -/
theorem call_sol_create_program_address_spec
    (r0Old vals ptr len pidA out a1 a2 n2 : Nat)
    (seedBytes pidBytes oldOut : ByteArray) (pc : Nat) (nCu : Nat)
    (ha1 : a1 = vals)
    (ha2 : a2 = vals + 8)
    (hn2 : n2 = 1)
    (hSeedSize : seedBytes.size = len)
    (hSeedLe : len ≤ 32)
    (hPidSize : pidBytes.size = 32)
    (hPtr : ptr < 2 ^ 64)
    (hLen : len < 2 ^ 64)
    (hOffCurve : ¬ Curve25519.validateEdwards
                   (Sha256.hash (seedBytes ++ pidBytes ++ Pda.PDA_MARKER)) = true)
    (hDescSeed : vals + 16 ≤ ptr ∨ ptr + len ≤ vals)
    (hDescPid  : vals + 16 ≤ pidA ∨ pidA + 32 ≤ vals)
    (hDescOut  : vals + 16 ≤ out ∨ out + 32 ≤ vals)
    (hSeedPid  : ptr + len ≤ pidA ∨ pidA + 32 ≤ ptr)
    (hSeedOut  : ptr + len ≤ out ∨ out + 32 ≤ ptr)
    (hPidOut   : pidA + 32 ≤ out ∨ out + 32 ≤ pidA)
    (hCu : ∀ s : State,
        (step (.call .sol_create_program_address) s).cuConsumed ≤ s.cuConsumed + nCu) :
    cuTripleWithinMem 1 nCu pc (pc + 1)
      (CodeReq.singleton pc (.call .sol_create_program_address))
      ((.r0 ↦ᵣ r0Old) ** (.r1 ↦ᵣ vals) ** (.r2 ↦ᵣ n2) ** (.r3 ↦ᵣ pidA) **
        (.r4 ↦ᵣ out) ** (a1 ↦U64 ptr) ** (a2 ↦U64 len) **
        (ptr ↦Bytes seedBytes) ** (pidA ↦Bytes32 pidBytes) ** (out ↦Bytes32 oldOut))
      ((.r0 ↦ᵣ 0) ** (.r1 ↦ᵣ vals) ** (.r2 ↦ᵣ n2) ** (.r3 ↦ᵣ pidA) **
        (.r4 ↦ᵣ out) ** (a1 ↦U64 ptr) ** (a2 ↦U64 len) **
        (ptr ↦Bytes seedBytes) ** (pidA ↦Bytes32 pidBytes) **
        (out ↦Bytes32 (Sha256.hash (seedBytes ++ pidBytes ++ Pda.PDA_MARKER))))
      (fun rt => (((rt.containsRange pidA 32 = true ∧
                    rt.containsWritable out 32 = true) ∧
                    rt.containsRange vals 16 = true) ∧
                    rt.containsRange ptr len = true)) := by
  subst a1; subst a2; subst n2
  intro R hRfree fetch hcr s hPR hpc hex hbud h_region
  ...
  obtain ⟨h_d1, h_T6, hd_d1_T6, hu_d1_T6, h_d1_pred, h_T6_sat⟩ := h_T5_sat
  obtain ⟨h_d2, h_T7, hd_d2_T7, hu_d2_T7, h_d2_pred, h_T7_sat⟩ := h_T6_sat
  obtain ⟨h_in, h_T8, hd_in_T8, hu_in_T8, h_in_pred, h_T8_sat⟩ := h_T7_sat
  obtain ⟨h_pid, h_out, hd_pid_out, hu_pid_out, h_pid_pred, h_out_pred⟩ := h_T8_sat
  rw [h_r0_pred] at hu_r0_T1 hd_r0_T1
  rw [h_r1_pred] at hu_r1_T2 hd_r1_T2
  rw [h_r2_pred] at hu_r2_T3 hd_r2_T3
  rw [h_r3_pred] at hu_r3_T4 hd_r3_T4
  rw [h_r4_pred] at hu_r4_T5 hd_r4_T5
  rw [h_d1_pred] at hu_d1_T6 hd_d1_T6
  rw [h_d2_pred] at hu_d2_T7 hd_d2_T7
  rw [h_in_pred] at hu_in_T8 hd_in_T8
  rw [h_pid_pred] at hu_pid_out hd_pid_out
  rw [h_out_pred] at hu_pid_out hd_pid_out
  clear h_r0_pred h_r1_pred h_r2_pred h_r3_pred h_r4_pred h_d1_pred h_d2_pred
        h_in_pred h_pid_pred h_out_pred
        h_r0 h_r1 h_r2 h_r3 h_r4 h_d1 h_d2 h_in h_pid h_out
  have hcr_regs := hcompat.regs
  have hcm_mem := hcompat.mem
  -- region requirements (envelope order: pid, out, descriptor, seed)
  have hPidReg : s.regions.containsRange pidA 32 = true := h_region.1.1.1
  have hWr : s.regions.containsWritable out 32 = true := h_region.1.1.2
  have hRangeVals : s.regions.containsRange vals 16 = true := h_region.1.2
  have hRangePtr : s.regions.containsRange ptr len = true := h_region.2
  -- climb the five registers (r4 at h_T4, r3 at h_T3, r2 at h_T2, r1 at h_T1, r0 at h_P)
  have h_T4_regs_r4 : h_T4.regs .r4 = some out := by
    rw [← hu_r4_T5]
    exact PartialState.union_regs_of_left_some PartialState.singletonReg_regs_self
  have h_T3_regs_r3 : h_T3.regs .r3 = some pidA := by
    rw [← hu_r3_T4]
    exact PartialState.union_regs_of_left_some PartialState.singletonReg_regs_self
  have h_T3_regs_r4 : h_T3.regs .r4 = some out := by
    rw [← hu_r3_T4,
        PartialState.union_regs_of_left_none
  ...
  have h_T2_regs_r2 : h_T2.regs .r2 = some 1 := by
    rw [← hu_r2_T3]
    exact PartialState.union_regs_of_left_some PartialState.singletonReg_regs_self
  have h_T2_regs_r3 : h_T2.regs .r3 = some pidA := by
    rw [← hu_r2_T3,
        PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r3 ≠ Reg.r2))]
    exact h_T3_regs_r3
  have h_T2_regs_r4 : h_T2.regs .r4 = some out := by
    rw [← hu_r2_T3,
        PartialState.union_regs_of_left_none
  ...
  have h_T1_regs_r2 : h_T1.regs .r2 = some 1 := by
    rw [← hu_r1_T2,
        PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r2 ≠ Reg.r1))]
    exact h_T2_regs_r2
  have h_T1_regs_r3 : h_T1.regs .r3 = some pidA := by
    rw [← hu_r1_T2,
        PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r3 ≠ Reg.r1))]
    exact h_T2_regs_r3
  have h_T1_regs_r4 : h_T1.regs .r4 = some out := by
    rw [← hu_r1_T2,
        PartialState.union_regs_of_left_none
  ...
  have h_P_regs_r2 : h_P.regs .r2 = some 1 := by
    rw [← hu_r0_T1,
        PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r2 ≠ Reg.r0))]
    exact h_T1_regs_r2
  have h_P_regs_r3 : h_P.regs .r3 = some pidA := by
    rw [← hu_r0_T1,
        PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r3 ≠ Reg.r0))]
    exact h_T1_regs_r3
  have h_P_regs_r4 : h_P.regs .r4 = some out := by
    rw [← hu_r0_T1,
        PartialState.union_regs_of_left_none
  ...
  have hp_regs_r2 : hp.regs .r2 = some 1 := by
    rw [← hu_PR]; exact PartialState.union_regs_of_left_some h_P_regs_r2
  have hp_regs_r3 : hp.regs .r3 = some pidA := by
    rw [← hu_PR]; exact PartialState.union_regs_of_left_some h_P_regs_r3
  have hp_regs_r4 : hp.regs .r4 = some out := by
    rw [← hu_PR]; exact PartialState.union_regs_of_left_some h_P_regs_r4
  have hs_regs_r0 : s.regs.get .r0 = r0Old := hcr_regs .r0 r0Old hp_regs_r0
  have hs_r1_field : s.regs.r1 = vals := hcr_regs .r1 vals hp_regs_r1
  have hs_r2_field : s.regs.r2 = 1 := hcr_regs .r2 1 hp_regs_r2
  have hs_r3_field : s.regs.r3 = pidA := hcr_regs .r3 pidA hp_regs_r3
  have hs_r4_field : s.regs.r4 = out := hcr_regs .r4 out hp_regs_r4
  -- generic climb: h_P.mem a from h_T5.mem a (past the five registers)
  have climb5 (a : Nat) : h_P.mem a = h_T5.mem a := by
    rw [← hu_r0_T1, PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a),
        ← hu_r1_T2, PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a),
        ← hu_r2_T3, PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a),
        ← hu_r3_T4, PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a),
        ← hu_r4_T5, PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a)]
  have h_P_to_s : ∀ a v, h_P.mem a = some v → s.mem a = v := by
    intro a v hav
    apply hcm_mem a v
    rw [← hu_PR]
  ...
  have hReadVals : Memory.readU64 s.mem vals = ptr := by
    ...
    · exact h_P_to_s _ _ (d1_in_P _ _ (PartialState.singletonMemU64_mem_6 vals ptr))
    · exact h_P_to_s _ _ (d1_in_P _ _ (PartialState.singletonMemU64_mem_7 vals ptr))
  -- descriptor cell 2 (at vals+8, past d1)
  have d2_in_P : ∀ a v, (PartialState.singletonMemU64 (vals + 8) len).mem a = some v →
      (vals + 8 ≤ a ∧ a < vals + 8 + 8) → h_P.mem a = some v := by
    intro a v hav ha
    rw [climb5, ← hu_d1_T6,
  ...
  have hReadVals8 : Memory.readU64 s.mem (vals + 8) = len := by
    ...
    · exact h_P_to_s _ _ (d2_in_P _ _ (PartialState.singletonMemU64_mem_6 (vals+8) len) (by omega))
    · exact h_P_to_s _ _ (d2_in_P _ _ (PartialState.singletonMemU64_mem_7 (vals+8) len) (by omega))
  -- seed slice (at ptr, past both descriptor cells)
  have in_in_P : ∀ i, i < seedBytes.size →
      h_P.mem (ptr + i) = some (seedBytes.get! i).toNat := by
    intro i hi
    rw [climb5, ← hu_d1_T6,
        PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside vals ptr (ptr + i) (by omega)),
        ...
        ← hu_in_T8]
    exact PartialState.union_mem_of_left_some
      (PartialState.singletonMemBytes_mem_at ptr seedBytes i hi)
  have hReadSeed : readBytes s.mem ptr len = seedBytes := by
    apply readBytes_eq_of_match s.mem ptr len seedBytes hSeedSize
    intro i hi
    have hi' : i < seedBytes.size := by rw [hSeedSize]; exact hi
    rw [h_P_to_s (ptr + i) _ (in_in_P i hi')]
    exact Nat.mod_eq_of_lt (seedBytes.get! i).toNat_lt
  -- program_id (at pidA, past d1, d2, seed)
  have pid_in_P : ∀ i, i < 32 →
      h_P.mem (pidA + i) = some (pidBytes.get! i).toNat := by
    intro i hi
    rw [climb5, ← hu_d1_T6,
        PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside vals ptr (pidA + i) (by omega)),
        ← hu_d2_T7,
        PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside (vals + 8) len (pidA + i) (by omega)),
        ← hu_in_T8,
        PartialState.union_mem_of_left_none
          (PartialState.singletonMemBytes_mem_outside ptr seedBytes (pidA + i) (by omega)),
        ← hu_pid_out]
    exact PartialState.union_mem_of_left_some
      (PartialState.singletonMem32Bytes_mem_at pidA pidBytes i hi)
  have hReadPid : readBytes s.mem pidA 32 = pidBytes := by
    apply readBytes_eq_of_match s.mem pidA 32 pidBytes hPidSize
    intro i hi
    rw [h_P_to_s (pidA + i) _ (pid_in_P i hi)]
    exact Nat.mod_eq_of_lt (pidBytes.get! i).toNat_lt
  -- the derived PDA + the execCreate collapse via the committed reductions
  have hReadSeedsEq : Pda.readSeeds s.mem vals 1 = [seedBytes] := by
    rw [Pda.readSeeds_one, hReadVals, hReadVals8, hReadSeed]
  have hcpa : Pda.createProgramAddress (Pda.readSeeds s.mem vals 1)
      (readBytes s.mem pidA 32)
      = some (Sha256.hash (seedBytes ++ pidBytes ++ Pda.PDA_MARKER)) := by
    rw [hReadSeedsEq, hReadPid]
    exact Pda.createProgramAddress_single seedBytes pidBytes
      (by rw [hSeedSize] at *; exact hSeedLe) hPidSize hOffCurve
  have hExecEq : Pda.execCreate s =
      { s with regs := s.regs.set .r0 0,
               mem := writeBytes s.mem out 32
                 (Sha256.hash (seedBytes ++ pidBytes ++ Pda.PDA_MARKER)) } := by
    apply Pda.execCreate_success_single s pidA out vals _
      hs_r3_field hs_r4_field hs_r1_field hs_r2_field hcpa hPidReg hWr hRangeVals
    intro i hi
    have hi0 : i = 0 := by simpa [List.mem_range, Nat.lt_one_iff] using hi
    subst hi0
    right
  ...
  have hstep_eq : executeFn fetch s 1
    ...
    rw [show (1 : Nat) = 0 + 1 from rfl,
        executeFn_step fetch s 0 _ hex (by omega) hfetch, executeFn_zero]
  have hexec_regs : (executeFn fetch s 1).regs = s.regs.set .r0 0 := by
    rw [hstep_eq]; show (Pda.execCreate s).regs = s.regs.set .r0 0; rw [hExecEq]
  have hexec_pc : (executeFn fetch s 1).pc = s.pc + 1 := by rw [hstep_eq]; rfl
  have hexec_exit : (executeFn fetch s 1).exitCode = none := by
    rw [hstep_eq]; show (Pda.execCreate s).exitCode = none; rw [hExecEq]; exact hex
  have hexec_rd : (executeFn fetch s 1).returnData = s.returnData := by
    rw [hstep_eq]; show (Pda.execCreate s).returnData = s.returnData; rw [hExecEq]
  have hexec_cs : (executeFn fetch s 1).callStack = s.callStack := by
    rw [hstep_eq]; show (Pda.execCreate s).callStack = s.callStack; rw [hExecEq]
  have hexec_mem_out (i : Nat) (hi : i < 32) :
      (executeFn fetch s 1).mem (out + i)
        = ((Sha256.hash (seedBytes ++ pidBytes ++ Pda.PDA_MARKER)).get! i).toNat := by
    rw [hstep_eq]; show (Pda.execCreate s).mem (out + i) = _; rw [hExecEq]
    exact writeBytes_read_inside s.mem out 32 i _ hi
  have hexec_mem_outside (a : Nat) (h : a < out ∨ a ≥ out + 32) :
      (executeFn fetch s 1).mem a = s.mem a := by
    rw [hstep_eq]; show (Pda.execCreate s).mem a = s.mem a; rw [hExecEq]
    exact writeBytes_read_outside s.mem out 32 a _ h
  -- s-side recovery shared by the compat proof
  have hs_regs_r1 : s.regs.get .r1 = vals := hs_r1_field
  have hs_regs_r2 : s.regs.get .r2 = 1 := hs_r2_field
  have hs_regs_r3 : s.regs.get .r3 = pidA := hs_r3_field
  have hs_regs_r4 : s.regs.get .r4 = out := hs_r4_field
  have hs_d1 : ∀ a v, (PartialState.singletonMemU64 vals ptr).mem a = some v →
      s.mem a = v := fun a v h => h_P_to_s a v (d1_in_P a v h)
  have hs_d2 : ∀ a v, (PartialState.singletonMemU64 (vals + 8) len).mem a = some v →
      (vals + 8 ≤ a ∧ a < vals + 8 + 8) → s.mem a = v :=
    fun a v h ha => h_P_to_s a v (d2_in_P a v h ha)
  have hs_in : ∀ i, i < seedBytes.size →
      s.mem (ptr + i) = (seedBytes.get! i).toNat :=
    fun i hi => h_P_to_s (ptr + i) _ (in_in_P i hi)
  have hs_pid : ∀ i, i < 32 → s.mem (pidA + i) = (pidBytes.get! i).toNat :=
    fun i hi => h_P_to_s (pidA + i) _ (pid_in_P i hi)
  -- ===== build the post heap (10 atoms) =====
  let h_r0_new : PartialState := PartialState.singletonReg .r0 0
  let h_r1_new : PartialState := PartialState.singletonReg .r1 vals
  let h_r2_new : PartialState := PartialState.singletonReg .r2 1
  let h_r3_new : PartialState := PartialState.singletonReg .r3 pidA
  let h_r4_new : PartialState := PartialState.singletonReg .r4 out
  let h_d1_new : PartialState := PartialState.singletonMemU64 vals ptr
  let h_d2_new : PartialState := PartialState.singletonMemU64 (vals + 8) len
  let h_in_new : PartialState := PartialState.singletonMemBytes ptr seedBytes
  let h_pid_new : PartialState := PartialState.singletonMem32Bytes pidA pidBytes
  let h_out_new : PartialState := PartialState.singletonMem32Bytes out
    (Sha256.hash (seedBytes ++ pidBytes ++ Pda.PDA_MARKER))
  let h_T8_new : PartialState := h_pid_new.union h_out_new
  let h_T7_new : PartialState := h_in_new.union h_T8_new
  let h_T6_new : PartialState := h_d2_new.union h_T7_new
  let h_T5_new : PartialState := h_d1_new.union h_T6_new
  let h_T4_new : PartialState := h_r4_new.union h_T5_new
  let h_T3_new : PartialState := h_r3_new.union h_T4_new
  let h_T2_new : PartialState := h_r2_new.union h_T3_new
  let h_T1_new : PartialState := h_r1_new.union h_T2_new
  let h_P_new : PartialState := h_r0_new.union h_T1_new
  -- program_id ⫫ output (both Bytes32)
  have hd_pid_out_new : h_pid_new.Disjoint h_out_new :=
    { regs := fun r => Or.inl (PartialState.singletonMem32Bytes_regs r)
      mem := fun a => by
        by_cases ha : pidA ≤ a ∧ a < pidA + 32
        · right
          obtain ⟨h_lo, h_hi⟩ := ha
          exact PartialState.singletonMem32Bytes_mem_outside out _ a (by omega)
        · left
          apply PartialState.singletonMem32Bytes_mem_outside
          rcases Nat.lt_or_ge a pidA with h' | h'
          · left; exact h'
          · rcases Nat.lt_or_ge a (pidA + 32) with h'' | h''
            · exact absurd ⟨h', h''⟩ ha
            · right; exact h''
  ...
  -- seed ⫫ (pid ∪ out)
  have hd_in_T8_new : h_in_new.Disjoint h_T8_new :=
    { regs := fun r => Or.inl (PartialState.singletonMemBytes_regs r)
      mem := fun a => by
        by_cases ha : ptr ≤ a ∧ a < ptr + seedBytes.size
        · right
          obtain ⟨h_lo, h_hi⟩ := ha
          show (h_pid_new.union h_out_new).mem a = none
          rw [PartialState.union_mem_of_left_none
                (PartialState.singletonMem32Bytes_mem_outside pidA pidBytes a (by omega))]
          exact PartialState.singletonMem32Bytes_mem_outside out _ a (by omega)
        · left
          apply PartialState.singletonMemBytes_mem_outside
          rcases Nat.lt_or_ge a ptr with h' | h'
          · left; exact h'
          · rcases Nat.lt_or_ge a (ptr + seedBytes.size) with h'' | h''
            · exact absurd ⟨h', h''⟩ ha
            · right; exact h''
      pc := Or.inl PartialState.singletonMemBytes_pc
      returnData := Or.inl PartialState.singletonMemBytes_returnData
      callStack := Or.inl PartialState.singletonMemBytes_callStack }
  -- d2 ⫫ (seed ∪ pid ∪ out)
  have hd_d2_T7_new : h_d2_new.Disjoint h_T7_new :=
    { regs := fun r => Or.inl (PartialState.singletonMemU64_regs r)
      mem := fun a => by
        by_cases ha : vals + 8 ≤ a ∧ a < vals + 8 + 8
        · right
          obtain ⟨h_lo, h_hi⟩ := ha
          show (h_in_new.union h_T8_new).mem a = none
          rw [PartialState.union_mem_of_left_none
                (PartialState.singletonMemBytes_mem_outside ptr seedBytes a (by omega))]
          show (h_pid_new.union h_out_new).mem a = none
          rw [PartialState.union_mem_of_left_none
                (PartialState.singletonMem32Bytes_mem_outside pidA pidBytes a (by omega))]
          exact PartialState.singletonMem32Bytes_mem_outside out _ a (by omega)
      ...
      callStack := Or.inl PartialState.singletonMemU64_callStack }
  -- d1 ⫫ (d2 ∪ seed ∪ pid ∪ out)
  have hd_d1_T6_new : h_d1_new.Disjoint h_T6_new :=
    { regs := fun r => Or.inl (PartialState.singletonMemU64_regs r)
      mem := fun a => by
        by_cases ha : vals ≤ a ∧ a < vals + 8
        · right
          obtain ⟨h_lo, h_hi⟩ := ha
          show (h_d2_new.union h_T7_new).mem a = none
          rw [PartialState.union_mem_of_left_none
                (PartialState.singletonMemU64_mem_outside (vals + 8) len a (by omega))]
          show (h_in_new.union h_T8_new).mem a = none
          rw [PartialState.union_mem_of_left_none
      ...
      returnData := Or.inl PartialState.singletonMemU64_returnData
      callStack := Or.inl PartialState.singletonMemU64_callStack }
  -- all-mem regs-none helper (h_T5_new onward is descriptor + seed + pid + out)
  have hT5new_regs_none (r : Reg) : h_T5_new.regs r = none := by
    show (h_d1_new.union h_T6_new).regs r = none
    rw [PartialState.union_regs_of_left_none (PartialState.singletonMemU64_regs r)]
    show (h_d2_new.union h_T7_new).regs r = none
    rw [PartialState.union_regs_of_left_none (PartialState.singletonMemU64_regs r)]
    show (h_in_new.union h_T8_new).regs r = none
    rw [PartialState.union_regs_of_left_none (PartialState.singletonMemBytes_regs r)]
    show (h_pid_new.union h_out_new).regs r = none
  ...
  have hd_r4_T5_new : h_r4_new.Disjoint h_T5_new := by
    refine
      { regs := fun r => ?_
        ...
        callStack := Or.inl PartialState.singletonReg_callStack }
    by_cases hr : r = .r4
    · right; rw [hr]; exact hT5new_regs_none .r4
    · left; exact PartialState.singletonReg_regs_other hr
  have hT4new_regs_other (r : Reg) (hr : r ≠ .r4) : h_T4_new.regs r = none := by
    show (h_r4_new.union h_T5_new).regs r = none
    rw [PartialState.union_regs_of_left_none (PartialState.singletonReg_regs_other hr)]
    exact hT5new_regs_none r
  have hd_r3_T4_new : h_r3_new.Disjoint h_T4_new := by
    refine
      { regs := fun r => ?_
        mem := fun a => Or.inl (PartialState.singletonReg_mem a)
        ...
        callStack := Or.inl PartialState.singletonReg_callStack }
    by_cases hr : r = .r3
    · right; rw [hr]; exact hT4new_regs_other .r3 (by decide)
    · left; exact PartialState.singletonReg_regs_other hr
  have hT3new_regs_other (r : Reg) (h3 : r ≠ .r3) (h4 : r ≠ .r4) :
      h_T3_new.regs r = none := by
    show (h_r3_new.union h_T4_new).regs r = none
    rw [PartialState.union_regs_of_left_none (PartialState.singletonReg_regs_other h3)]
    exact hT4new_regs_other r h4
  have hd_r2_T3_new : h_r2_new.Disjoint h_T3_new := by
    refine
      { regs := fun r => ?_
        mem := fun a => Or.inl (PartialState.singletonReg_mem a)
        ...
        callStack := Or.inl PartialState.singletonReg_callStack }
    by_cases hr : r = .r2
    · right; rw [hr]; exact hT3new_regs_other .r2 (by decide) (by decide)
    · left; exact PartialState.singletonReg_regs_other hr
  have hT2new_regs_other (r : Reg) (h2 : r ≠ .r2) (h3 : r ≠ .r3) (h4 : r ≠ .r4) :
      h_T2_new.regs r = none := by
    show (h_r2_new.union h_T3_new).regs r = none
    rw [PartialState.union_regs_of_left_none (PartialState.singletonReg_regs_other h2)]
    exact hT3new_regs_other r h3 h4
  have hd_r1_T2_new : h_r1_new.Disjoint h_T2_new := by
    refine
      { regs := fun r => ?_
        mem := fun a => Or.inl (PartialState.singletonReg_mem a)
        ...
        callStack := Or.inl PartialState.singletonReg_callStack }
    by_cases hr : r = .r1
    · right; rw [hr]; exact hT2new_regs_other .r1 (by decide) (by decide) (by decide)
    · left; exact PartialState.singletonReg_regs_other hr
  have hT1new_regs_other (r : Reg) (h1 : r ≠ .r1) (h2 : r ≠ .r2) (h3 : r ≠ .r3)
      (h4 : r ≠ .r4) : h_T1_new.regs r = none := by
    show (h_r1_new.union h_T2_new).regs r = none
    rw [PartialState.union_regs_of_left_none (PartialState.singletonReg_regs_other h1)]
    exact hT2new_regs_other r h2 h3 h4
  have hd_r0_T1_new : h_r0_new.Disjoint h_T1_new := by
    refine
      { regs := fun r => ?_
        mem := fun a => Or.inl (PartialState.singletonReg_mem a)
    ...
    by_cases hr : r = .r0
    · right; rw [hr]
      exact hT1new_regs_other .r0 (by decide) (by decide) (by decide) (by decide)
    · left; exact PartialState.singletonReg_regs_other hr
  -- ===== per-field projections of h_P_new =====
  have h_P_new_regs_r0 : h_P_new.regs .r0 = some 0 := by
    show (h_r0_new.union h_T1_new).regs .r0 = some 0
  ...
  have h_P_new_regs_r2 : h_P_new.regs .r2 = some 1 := by
    ...
    show (h_r2_new.union h_T3_new).regs .r2 = some 1
    exact PartialState.union_regs_of_left_some PartialState.singletonReg_regs_self
  have h_P_new_regs_r3 : h_P_new.regs .r3 = some pidA := by
    show (h_r0_new.union h_T1_new).regs .r3 = some pidA
    rw [PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r3 ≠ Reg.r0))]
    show (h_r1_new.union h_T2_new).regs .r3 = some pidA
    rw [PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r3 ≠ Reg.r1))]
    show (h_r2_new.union h_T3_new).regs .r3 = some pidA
    rw [PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r3 ≠ Reg.r2))]
    show (h_r3_new.union h_T4_new).regs .r3 = some pidA
    exact PartialState.union_regs_of_left_some PartialState.singletonReg_regs_self
  have h_P_new_regs_r4 : h_P_new.regs .r4 = some out := by
    show (h_r0_new.union h_T1_new).regs .r4 = some out
    rw [PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r4 ≠ Reg.r0))]
    show (h_r1_new.union h_T2_new).regs .r4 = some out
    rw [PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r4 ≠ Reg.r1))]
    show (h_r2_new.union h_T3_new).regs .r4 = some out
    rw [PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r4 ≠ Reg.r2))]
    show (h_r3_new.union h_T4_new).regs .r4 = some out
    rw [PartialState.union_regs_of_left_none
          (PartialState.singletonReg_regs_other (by decide : Reg.r4 ≠ Reg.r3))]
    show (h_r4_new.union h_T5_new).regs .r4 = some out
    exact PartialState.union_regs_of_left_some PartialState.singletonReg_regs_self
  have h_P_new_regs_other (r : Reg)
    ...
    show (h_r0_new.union h_T1_new).regs r = none
    rw [PartialState.union_regs_of_left_none (PartialState.singletonReg_regs_other h0)]
    exact hT1new_regs_other r h1 h2 h3 h4
  -- mem projections (climb past 5 regs)
  have h_P_new_mem_d1 (a : Nat) (ha : vals ≤ a ∧ a < vals + 8) :
      h_P_new.mem a = (PartialState.singletonMemU64 vals ptr).mem a := by
    obtain ⟨x, hx⟩ := PartialState.singletonMemU64_mem_isSome vals ptr a ha
    rw [hx]
    ...
    show (h_r3_new.union h_T4_new).mem a = some x
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a)]
    show (h_r4_new.union h_T5_new).mem a = some x
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a)]
    show (h_d1_new.union h_T6_new).mem a = some x
    exact PartialState.union_mem_of_left_some hx
  have h_P_new_mem_d2 (a : Nat) (ha : vals + 8 ≤ a ∧ a < vals + 8 + 8) :
      h_P_new.mem a = (PartialState.singletonMemU64 (vals + 8) len).mem a := by
    obtain ⟨x, hx⟩ := PartialState.singletonMemU64_mem_isSome (vals + 8) len a ha
    ...
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside vals ptr a (by omega))]
    show (h_d2_new.union h_T7_new).mem a = some x
    exact PartialState.union_mem_of_left_some hx
  have h_P_new_mem_in (i : Nat) (hi : i < seedBytes.size) :
      h_P_new.mem (ptr + i) = some (seedBytes.get! i).toNat := by
    show (h_r0_new.union h_T1_new).mem (ptr + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_r1_new.union h_T2_new).mem (ptr + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    ...
    show (h_r3_new.union h_T4_new).mem (ptr + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_r4_new.union h_T5_new).mem (ptr + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_d1_new.union h_T6_new).mem (ptr + i) = _
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside vals ptr (ptr + i) (by omega))]
    show (h_d2_new.union h_T7_new).mem (ptr + i) = _
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside (vals + 8) len (ptr + i) (by omega))]
    show (h_in_new.union h_T8_new).mem (ptr + i) = _
    exact PartialState.union_mem_of_left_some
      (PartialState.singletonMemBytes_mem_at ptr seedBytes i hi)
  have h_P_new_mem_pid (i : Nat) (hi : i < 32) :
      h_P_new.mem (pidA + i) = some (pidBytes.get! i).toNat := by
    show (h_r0_new.union h_T1_new).mem (pidA + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_r1_new.union h_T2_new).mem (pidA + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_r2_new.union h_T3_new).mem (pidA + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_r3_new.union h_T4_new).mem (pidA + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_r4_new.union h_T5_new).mem (pidA + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_d1_new.union h_T6_new).mem (pidA + i) = _
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside vals ptr (pidA + i) (by omega))]
    show (h_d2_new.union h_T7_new).mem (pidA + i) = _
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside (vals + 8) len (pidA + i) (by omega))]
    show (h_in_new.union h_T8_new).mem (pidA + i) = _
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemBytes_mem_outside ptr seedBytes (pidA + i) (by omega))]
    show (h_pid_new.union h_out_new).mem (pidA + i) = _
    exact PartialState.union_mem_of_left_some
      (PartialState.singletonMem32Bytes_mem_at pidA pidBytes i hi)
  have h_P_new_mem_out (i : Nat) (hi : i < 32) :
      h_P_new.mem (out + i)
        = some ((Sha256.hash (seedBytes ++ pidBytes ++ Pda.PDA_MARKER)).get! i).toNat := by
    show (h_r0_new.union h_T1_new).mem (out + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_r1_new.union h_T2_new).mem (out + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    ...
    show (h_r3_new.union h_T4_new).mem (out + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_r4_new.union h_T5_new).mem (out + i) = _
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem _)]
    show (h_d1_new.union h_T6_new).mem (out + i) = _
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside vals ptr (out + i) (by omega))]
    show (h_d2_new.union h_T7_new).mem (out + i) = _
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside (vals + 8) len (out + i) (by omega))]
    show (h_in_new.union h_T8_new).mem (out + i) = _
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemBytes_mem_outside ptr seedBytes (out + i) (by omega))]
    show (h_pid_new.union h_out_new).mem (out + i) = _
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMem32Bytes_mem_outside pidA pidBytes (out + i) (by omega))]
    exact PartialState.singletonMem32Bytes_mem_at out _ i hi
  have h_P_new_mem_outside (a : Nat)
      (hd1 : a < vals ∨ a ≥ vals + 8) (hd2 : a < vals + 8 ∨ a ≥ vals + 8 + 8)
      (hin : a < ptr ∨ a ≥ ptr + seedBytes.size) (hpid : a < pidA ∨ a ≥ pidA + 32)
      (hou : a < out ∨ a ≥ out + 32) : h_P_new.mem a = none := by
    show (h_r0_new.union h_T1_new).mem a = none
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a)]
    show (h_r1_new.union h_T2_new).mem a = none
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a)]
    ...
    rw [PartialState.union_mem_of_left_none (PartialState.singletonReg_mem a)]
    show (h_d1_new.union h_T6_new).mem a = none
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemU64_mem_outside vals ptr a hd1)]
    show (h_d2_new.union h_T7_new).mem a = none
    rw [PartialState.union_mem_of_left_none
    ...
    show (h_in_new.union h_T8_new).mem a = none
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMemBytes_mem_outside ptr seedBytes a hin)]
    show (h_pid_new.union h_out_new).mem a = none
    rw [PartialState.union_mem_of_left_none
          (PartialState.singletonMem32Bytes_mem_outside pidA pidBytes a hpid)]
    exact PartialState.singletonMem32Bytes_mem_outside out _ a hou
  have h_P_new_pc : h_P_new.pc = none := by
    show (h_r0_new.union h_T1_new).pc = none
    rw [PartialState.union_pc_of_left_none PartialState.singletonReg_pc]
    ...
    rw [PartialState.union_pc_of_left_none PartialState.singletonReg_pc]
    show (h_d1_new.union h_T6_new).pc = none
    rw [PartialState.union_pc_of_left_none PartialState.singletonMemU64_pc]
    show (h_d2_new.union h_T7_new).pc = none
    rw [PartialState.union_pc_of_left_none PartialState.singletonMemU64_pc]
    show (h_in_new.union h_T8_new).pc = none
    rw [PartialState.union_pc_of_left_none PartialState.singletonMemBytes_pc]
    show (h_pid_new.union h_out_new).pc = none
  ...
  have h_P_new_rd : h_P_new.returnData = none := by
    ...
    show (h_r3_new.union h_T4_new).returnData = none
    rw [PartialState.union_returnData_of_left_none PartialState.singletonReg_returnData]
    show (h_r4_new.union h_T5_new).returnData = none
    rw [PartialState.union_returnData_of_left_none PartialState.singletonReg_returnData]
    show (h_d1_new.union h_T6_new).returnData = none
    rw [PartialState.union_returnData_of_left_none PartialState.singletonMemU64_returnData]
    show (h_d2_new.union h_T7_new).returnData = none
    rw [PartialState.union_returnData_of_left_none PartialState.singletonMemU64_returnData]
    show (h_in_new.union h_T8_new).returnData = none
    rw [PartialState.union_returnData_of_left_none PartialState.singletonMemBytes_returnData]
    show (h_pid_new.union h_out_new).returnData = none
  ...
  have h_P_new_cs : h_P_new.callStack = none := by
    ...
    show (h_r3_new.union h_T4_new).callStack = none
    rw [PartialState.union_callStack_of_left_none PartialState.singletonReg_callStack]
    show (h_r4_new.union h_T5_new).callStack = none
    rw [PartialState.union_callStack_of_left_none PartialState.singletonReg_callStack]
    show (h_d1_new.union h_T6_new).callStack = none
    rw [PartialState.union_callStack_of_left_none PartialState.singletonMemU64_callStack]
    show (h_d2_new.union h_T7_new).callStack = none
    rw [PartialState.union_callStack_of_left_none PartialState.singletonMemU64_callStack]
    show (h_in_new.union h_T8_new).callStack = none
    rw [PartialState.union_callStack_of_left_none PartialState.singletonMemBytes_callStack]
    show (h_pid_new.union h_out_new).callStack = none
    rw [PartialState.union_callStack_of_left_none PartialState.singletonMem32Bytes_callStack]
    exact PartialState.singletonMem32Bytes_callStack
  -- ===== h_R absence facts =====
  have hd_PR_regs := hd_PR.regs
  have hd_PR_mem := hd_PR.mem
  ...
  have hR_no_r3 : h_R.regs .r3 = none := by
    ...
    · rw [h_P_regs_r3] at h; nomatch h
    · exact h
  have hR_no_r4 : h_R.regs .r4 = none := by
    rcases hd_PR_regs .r4 with h | h
    · rw [h_P_regs_r4] at h; nomatch h
    · exact h
  have hR_no_pc : h_R.pc = none := hRfree _ h_R_sat
  have hR_no_mem_d1 (a : Nat) (ha : vals ≤ a ∧ a < vals + 8) : h_R.mem a = none := by
    obtain ⟨x, hx⟩ := PartialState.singletonMemU64_mem_isSome vals ptr a ha
  ...
  have hR_no_mem_d2 (a : Nat) (ha : vals + 8 ≤ a ∧ a < vals + 8 + 8) :
    ...
    · rw [d2_in_P a x hx ha] at h; nomatch h
    · exact h
  have hR_no_mem_in (i : Nat) (hi : i < seedBytes.size) : h_R.mem (ptr + i) = none := by
    rcases hd_PR_mem (ptr + i) with h | h
    · rw [in_in_P i hi] at h; nomatch h
    · exact h
  have hR_no_mem_pid (i : Nat) (hi : i < 32) : h_R.mem (pidA + i) = none := by
    rcases hd_PR_mem (pidA + i) with h | h
    · rw [pid_in_P i hi] at h; nomatch h
    · exact h
  -- the pre-state output cell (held `oldOut`) is owned by h_P
  have out_in_P (i : Nat) (hi : i < 32) :
      h_P.mem (out + i) = some (oldOut.get! i).toNat := by
    rw [climb5, ← hu_d1_T6,
        PartialState.union_mem_of_left_none
  ...
  have hd_PnewR : h_P_new.Disjoint h_R :=
    { regs := fun r => by
      ...
      mem := fun a => by
        ...
        by_cases hd2 : vals + 8 ≤ a ∧ a < vals + 8 + 8
        · right; exact hR_no_mem_d2 a hd2
        by_cases hin : ptr ≤ a ∧ a < ptr + seedBytes.size
        · right
          obtain ⟨h_lo, h_hi⟩ := hin
          rw [show a = ptr + (a - ptr) from by omega]
          exact hR_no_mem_in (a - ptr) (by omega)
        by_cases hpid : pidA ≤ a ∧ a < pidA + 32
        · right
          obtain ⟨h_lo, h_hi⟩ := hpid
          rw [show a = pidA + (a - pidA) from by omega]
          exact hR_no_mem_pid (a - pidA) (by omega)
        · left
          apply h_P_new_mem_outside
          · rcases Nat.lt_or_ge a vals with h | h
            · left; exact h
  ...
  · refine ⟨h_P_new.union h_R, ?_, h_P_new, h_R, hd_PnewR, rfl,
            ⟨h_r0_new, h_T1_new, hd_r0_T1_new, rfl, rfl,
             ...
             h_d1_new, h_T6_new, hd_d1_T6_new, rfl, rfl,
             h_d2_new, h_T7_new, hd_d2_T7_new, rfl, rfl,
             h_in_new, h_T8_new, hd_in_T8_new, rfl, rfl,
             h_pid_new, h_out_new, hd_pid_out_new, rfl, rfl, rfl⟩,
            h_R_sat⟩
    refine
      { regs := ?_, mem := ?_, pc := ?_, returnData := ?_, callStack := ?_ }
    · -- regs compat
      ...
      · rw [h4] at hvr
        ...
        rw [h4, hexec_regs, ← (Option.some.inj hvr),
    ...
    · -- mem compat
      ...
      · obtain ⟨h_lo, h_hi⟩ := hin
        have h_eq : a = ptr + (a - ptr) := by omega
        have h_lt : a - ptr < seedBytes.size := by omega
        rw [h_eq] at hva ⊢
        rw [PartialState.union_mem_of_left_some (h_P_new_mem_in _ h_lt)] at hva
        rw [hexec_mem_outside (ptr + (a - ptr)) (by omega), hs_in _ h_lt,
            ← (Option.some.inj hva)]
      by_cases hpid : pidA ≤ a ∧ a < pidA + 32
      · obtain ⟨h_lo, h_hi⟩ := hpid
        have h_eq : a = pidA + (a - pidA) := by omega
        have h_lt : a - pidA < 32 := by omega
        rw [h_eq] at hva ⊢
        rw [PartialState.union_mem_of_left_some (h_P_new_mem_pid _ h_lt)] at hva
        rw [hexec_mem_outside (pidA + (a - pidA)) (by omega), hs_pid _ h_lt,
            ← (Option.some.inj hva)]
      · -- outside all owned regions: value comes from the frame
        have h_out_addr : a < out ∨ a ≥ out + 32 := by
          rcases Nat.lt_or_ge a out with h | h
              ...
              (h_P_new_mem_outside a
                ...
                (by rcases Nat.lt_or_ge a ptr with h | h
                    ...
                    · rcases Nat.lt_or_ge a (ptr + seedBytes.size) with h' | h'
                      · exact absurd ⟨h, h'⟩ hin
                      · exact Or.inr h')
                (by rcases Nat.lt_or_ge a pidA with h | h
                    · exact Or.inl h
                    · rcases Nat.lt_or_ge a (pidA + 32) with h' | h'
                      · exact absurd ⟨h, h'⟩ hpid
                      · exact Or.inr h')
    ...
    · -- returnData compat
      ...
      have h_P_rd_pre : h_P.returnData = none := by
        rw [← hu_r0_T1, ← hu_r1_T2, ← hu_r2_T3, ← hu_r3_T4, ← hu_r4_T5,
            ← hu_d1_T6, ← hu_d2_T7, ← hu_in_T8, ← hu_pid_out]
        rfl
      have hp_rd : hp.returnData = some rd := by
        rw [← hu_PR, PartialState.union_returnData_of_left_none h_P_rd_pre]
        exact hva
