...
/-! ## Byte width — decomposition lemmas + `ldx/stx/st .byte`

Same instantiation pattern as the wider widths, with
`M := PartialState.singletonMem ..` (one byte cell). NOTE (L3):
`singletonMem` stores its value RAW, so the store instantiations pass
the pre-masked value (`vSrc % 256`, `toU64 imm % 256`) as both the
atom's value and the written value. -/

theorem PartialState.singletonMem_memOnly (a v : Nat) :
    (PartialState.singletonMem a v).MemOnly :=
  ⟨fun _ => rfl, rfl, rfl, rfl⟩

/-- `writeU8 _ addr v` reproduces the byte owned by `singletonMem addr v`
    (`v < 256`, as produced by every masked byte store). -/
theorem singletonMem_write_in (addr v : Nat) (hlt : v < 256) :
    ∀ (m : Memory.Mem) (a val : Nat),
      (PartialState.singletonMem addr v).mem a = some val →
      (Memory.writeU8 m addr v) a = val := by
  intro m a val h
  by_cases h0 : a = addr
  · subst h0
    have hval : val = v := by
      rw [PartialState.singletonMem_mem_self] at h
      exact (Option.some.inj h).symm
    rw [hval, Memory.writeU8_read_at]
    omega
  · rw [PartialState.singletonMem_mem_other h0] at h
    nomatch h

/-- `writeU8` leaves every byte outside `singletonMem`'s footprint unchanged. -/
theorem singletonMem_write_out (addr v w : Nat) :
    ∀ (m : Memory.Mem) (a : Nat),
      (PartialState.singletonMem addr v).mem a = none →
      (Memory.writeU8 m addr w) a = m a := by
  intro m a h
  by_cases h0 : a = addr
  · subst h0; rw [PartialState.singletonMem_mem_self] at h; nomatch h
  · exact Memory.writeU8_read_other m addr w a h0

/-- `singletonMem` atoms at the same address have the same footprint
    regardless of value. -/
theorem singletonMem_foot (addr v w : Nat) :
    ∀ a, (PartialState.singletonMem addr v).mem a = none ↔
         (PartialState.singletonMem addr w).mem a = none := by
  intro a
  by_cases h0 : a = addr
  · subst h0
    rw [PartialState.singletonMem_mem_self, PartialState.singletonMem_mem_self]
    exact ⟨fun h => (nomatch h), fun h => (nomatch h)⟩
  · rw [PartialState.singletonMem_mem_other h0,
        PartialState.singletonMem_mem_other h0]
...
theorem ldxb_spec
  ...
  cuTripleWithinMem_load_cell_via_reg_addr dst src vOldDst baseAddr
    (byteVal % 256) pc (.ldx .byte dst src off)
    (PartialState.singletonMem (effectiveAddr baseAddr off) byteVal) _ _
    hne (fun _ => Iff.rfl)
    (PartialState.singletonMem_memOnly _ _)
    (fun s _ hsrc hmem hreg => by
      simp only [step, hsrc, Width.bytes, if_pos hreg,
                 Memory.readByWidth, Memory.readU8,
                 hmem _ _ PartialState.singletonMem_mem_self])
...
theorem ldxb_same_spec
    ...
    cuTripleWithinMem 1 0 pc (pc + 1)
      ...
      (fun rt => rt.containsRange (effectiveAddr baseAddr off) 1 = true) :=
  cuTripleWithinMem_load_cell_same_reg r baseAddr (byteVal % 256) pc
    (.ldx .byte r r off)
    (PartialState.singletonMem (effectiveAddr baseAddr off) byteVal) _ _
    hne (fun _ => Iff.rfl)
    (PartialState.singletonMem_memOnly _ _)
...
theorem stxb_spec
  ...
  cuTripleWithinMem_store_cell_via_reg_addr baseReg valReg baseAddr vSrc pc
    (.stx .byte baseReg off valReg)
    (PartialState.singletonMem (effectiveAddr baseAddr off) oldByteVal)
    (PartialState.singletonMem (effectiveAddr baseAddr off) (vSrc % 256))
    _ _
    (fun m => Memory.writeU8 m (effectiveAddr baseAddr off) (vSrc % 256)) _
    (fun _ => Iff.rfl) (fun _ => Iff.rfl)
    (PartialState.singletonMem_memOnly _ _)
    (PartialState.singletonMem_memOnly _ _)
    (singletonMem_foot _ _ _)
    (singletonMem_write_in _ _ (by omega))
    (singletonMem_write_out _ _ _)
...
theorem stb_spec
  ...
  cuTripleWithinMem_store_imm_cell_via_reg_addr baseReg baseAddr pc
    (.st .byte baseReg off imm)
    (PartialState.singletonMem (effectiveAddr baseAddr off) oldByteVal)
    (PartialState.singletonMem (effectiveAddr baseAddr off) (toU64 imm % 256))
    _ _
    (fun m => Memory.writeU8 m (effectiveAddr baseAddr off) (toU64 imm % 256)) _
    (fun _ => Iff.rfl) (fun _ => Iff.rfl)
    (PartialState.singletonMem_memOnly _ _)
    (PartialState.singletonMem_memOnly _ _)
    (singletonMem_foot _ _ _)
