...
open SVM SVM.SBPF SVM.SBPF.Memory SVM.Pubkey SVM.Solana.Abstract
...
`rest` region as a `.blob` (the layout-general shape the vault-blob codegen
now emits). Stated directly here; the token lift converges to
`AsmRefinesFieldUpdate` in #24, at which point these wire through like the
vault above. -/
