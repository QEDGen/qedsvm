/-
  The first real-bytecode asm-refines-intrinsic theorem. Generated by
  ...
  `dst_account_eq` + `cuTripleWithinMem_frame_right` + `sl_exact`. This closes the
  fine→coarse SL field-aggregation step (the codec-wrap gating noted in the
  qedlift status).

  Two tactic fixes made this go through (both in `SVM/SBPF/Tactic/SL.lean`):
    1. `buildRightFoldIff` now flattens a *compound head* `A` of `(group ** rest)`
       — the codec's expanded pubkey group `(c0**c1**c2**c3)` — instead of
       treating it atomically, so it agrees with `flattenSepConj`'s full flatten.
    2. `sl_exact`/`buildMatchIff` inserts a per-atom `buildDefeqBridge`, ascribing
       each defeq-but-not-syntactic atom pair (the lift's `effectiveAddr base 160`
       vs the aggregation's `base + 160`) in isolation, so the kernel never faces
       one giant `isDefEq` over the ~50-atom sepConj (which it bailed on).
  The `simp only [Nat.add_assoc, Nat.reduceAdd]` before `sl_exact` folds the
  codec's nested account-base offsets (`baseAddr + 96 + k`) into single literals,
  keeping every residual defeq cheap.
-/
...
theorem refines_asm
  ...
  have framed := cuTripleWithinMem_frame_right
    ...
    (by sl_pcfree) lift
  -- Fold the codec-aggregation's nested account-base offsets into single
  -- literals: `baseAddr + 96 + k` ⟶ `baseAddr + (96+k)`. The lift carries the
  -- same locations as `effectiveAddr baseAddr OFF`. Both are defeq, but the
  -- nested-add form makes the `isDefEq` `sl_exact` needs to bridge them blow up
  -- (`Nat.add baseAddr 10632` vs `Nat.add (Nat.add baseAddr 10600) 32` forces a
  -- ~10k-step successor unfolding per dst atom). Folding the codec side to a
  -- single literal collapses that to a one-step `effectiveAddr → base + lit`
  -- reduction, which `sl_exact`'s term composition handles cheaply.
