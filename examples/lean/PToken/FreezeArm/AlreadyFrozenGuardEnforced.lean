/-
  ...
  p-token FreezeAccount arm: freezing an already-frozen account is
  rejected (freeze/thaw must alternate).

  From an already-frozen account (state byte = 2 — the VIOLATION as
  hypothesis, with a VALID freeze authority signing), the lifted error
  path runs to the shared exit with r0 = 13 (TokenError::InvalidState),
  the state byte untouched.
-/

import Generated.PTokenFreezeAlreadyFrozenLifted
import Lean
import SVM.Solana.Patterns.Guards

namespace Examples.PTokenFreezeAlreadyFrozenGuard

open SVM.SBPF
open Memory
open Examples.Lifted.PTokenFreezeAlreadyFrozen

set_option maxHeartbeats 1600000 in
/-- Freezing an already-frozen account, the real p-token FreezeAccount
    HALTS with `exitCode = some 13` (TokenError::InvalidState) — the
    state-transition check is ENFORCED. -/
theorem p_token_freeze_already_frozen_enforced
    ...
    : SVM.Solana.Patterns.EnforcedError (135 + 1) nCuLog23 198
...
#assert_std_axioms_local Examples.Lifted.PTokenFreezeAlreadyFrozen.PTokenFreezeAlreadyFrozen_lifted_spec
#assert_std_axioms_local Examples.PTokenFreezeAlreadyFrozenGuard.p_token_freeze_already_frozen_enforced

end Examples.PTokenFreezeAlreadyFrozenGuard
