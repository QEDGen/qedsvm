...
out = f"""\
/-
  Generated by `gen_transfer_refinement.py` (the codegen prototype for what
  qedlift will emit per-arm). Wires the real trace-guided Transfer lift to
  `AsmRefinesTokenTransfer` (the tokenTransfer intrinsic) via the SL bridges +
  `src_account_eq`/`dst_account_eq` + `cuTripleWithinMem_frame_right` + `sl_exact`.

  NOTE on scoping: this file imports `SVM.Solana.Abstract.Refinement` for
  `AsmRefinesTokenTransfer` but deliberately does NOT `open SVM.Solana.Abstract`.
  Abstract's `**` is `scoped`, so leaving it unopened keeps only `SVM.SBPF`'s
  `**` in scope — the ~48-atom separation-logic chains here then have an
  unambiguous `**` and elaborate in ~1.5s. Opening Abstract would bring a second
  `**` overload into scope and make overload resolution over the nested chains
  blow up (super-)exponentially (>13 min just on import). The few Abstract
  references are fully qualified instead.

  REMAINING: `sl_exact framed` still fails with an application type mismatch —
  the fine→coarse SL field-aggregation alignment (codec-wrap gating). Not perf.
-/
