/-
  Pattern library Layer 3 — the UNINITIALIZED-SOURCE guard's full ENFORCES
  composition on the real p-token binary.

  From an uninitialized source account (state byte = 0 — the VIOLATION as
  hypothesis), the mechanically lifted error path
  (`PTokenTransferSrcUninit_lifted_spec`) runs from the Transfer dispatch
  entry through the taken `jeq state, 0` at pc 4005, the UninitializedAccount
  error route, the logging helper, and the BUILTIN ProgramError encoder
  (unlike the TokenError customs, builtins encode in the HIGH 32 bits:
  r0 = 10 <<< 32 = 42949672960, ProgramError::UninitializedAccount) to the
  shared exit, token cells untouched. Composed with the `.exit` into the
  halting `EnforcedError`: exitCode = some 42949672960 ≠ 0.
-/

import Generated.PTokenTransferSrcUninitLifted
import Lean
import SVM.Solana.Patterns.Guards

namespace Examples.PTokenSrcUninitGuardEnforced

open SVM.SBPF
open Memory
open Examples.Lifted.PTokenTransferSrcUninit

set_option maxHeartbeats 1600000 in
/-- From an uninitialized source account, the real p-token Transfer HALTS
    with `exitCode = some (10 <<< 32)` (ProgramError::UninitializedAccount),
    the token cells untouched — the initialized check is ENFORCED, not merely
    required. -/
theorem p_token_src_uninit_enforced
    ...
    : SVM.Solana.Patterns.EnforcedError (57 + 1) nCuLog9 198
...
#assert_std_axioms_local Examples.Lifted.PTokenTransferSrcUninit.PTokenTransferSrcUninit_lifted_spec
#assert_std_axioms_local Examples.PTokenSrcUninitGuardEnforced.p_token_src_uninit_enforced

end Examples.PTokenSrcUninitGuardEnforced
