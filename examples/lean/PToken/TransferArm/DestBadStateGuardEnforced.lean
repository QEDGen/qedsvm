/-
  Pattern library Layer 3 — the INVALID-STATE-BYTE guard (destination),
  sibling of `SrcBadStateGuardEnforced.lean`: the source's `jgt state, 2`
  at 4004 falls through, the dest's at 4007 takes. r0 = 17179869184
  (ProgramError::InvalidAccountData).
-/

import Generated.PTokenTransferDestBadStateLifted
import Lean
import SVM.Solana.Patterns.Guards

namespace Examples.PTokenDestBadStateGuardEnforced

open SVM.SBPF
open Memory
open Examples.Lifted.PTokenTransferDestBadState

set_option maxHeartbeats 1600000 in
/-- From a destination account whose state byte is outside the AccountState
    tag range (> 2), the real p-token Transfer HALTS with
    `exitCode = some (4 <<< 32)` (ProgramError::InvalidAccountData) — the
    account-shape check is ENFORCED on the destination too. -/
theorem p_token_dest_bad_state_enforced
    ...
    (h_branch10 : ¬ toSigned64 ((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 12))
    (h_branch11 : toSigned64 ((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) ≤ toSigned64 (toU64 5))
    (h_branch12 : toSigned64 ((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 2))
    (h_branch13 : (((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) = toU64 3)
    (h_branch14 : ¬ toSigned64 ((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 12))
    (h_branch15 : toSigned64 ((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) ≤ toSigned64 (toU64 5))
    (h_branch16 : toSigned64 ((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 2))
    (h_branch17 : (((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) = toU64 3)
    (nCuLog10 : Nat)
    (hCuLog10 : ∀ s : State, (step (.call .sol_log_) s).cuConsumed ≤ s.cuConsumed + nCuLog10)
    : SVM.Solana.Patterns.EnforcedError (55 + 1) nCuLog10 198
...
#assert_std_axioms_local Examples.Lifted.PTokenTransferDestBadState.PTokenTransferDestBadState_lifted_spec
#assert_std_axioms_local Examples.PTokenDestBadStateGuardEnforced.p_token_dest_bad_state_enforced

end Examples.PTokenDestBadStateGuardEnforced
