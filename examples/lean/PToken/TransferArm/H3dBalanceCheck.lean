...
def h3dCr : CodeReq :=
  (((((((CodeReq.singleton 0 (.mov64 .r6 (.imm 0))).union
        (CodeReq.singleton 1 (.mov64 .r7 (.imm 0x11)))).union
        (CodeReq.singleton 2 (.jeq .r3 (.imm 2) h3dErrPc))).union
        (CodeReq.singleton 3 (.jeq .r5 (.imm 2) h3dErrPc))).union
        (CodeReq.singleton 4 (.ldx .dword .r2 .r2 0x7a81))).union
        (CodeReq.singleton 5 (.mov64 .r7 (.imm 1)))).union
        (CodeReq.singleton 6 (.ldx .dword .r3 .r1 0xa0))).union
        (CodeReq.singleton 7 (.jlt .r3 (.reg .r2) h3dErrPc))
...
theorem p_token_transfer_arm_h3d_spec
    ...
    cuTripleWithinMem 8 0 0 8 h3dCr
  ...
  have h0 := mov64_imm_spec .r6 0 (toU64 3) 0 (by decide)
  have h1 := mov64_imm_spec .r7 0x11 (toU64 0) 1 (by decide)
  have h2 := jeq_imm_spec .r3 2 (srcState % 256) 2 h3dErrPc
  have h3 := jeq_imm_spec .r5 2 (dstState % 256) 3 h3dErrPc
  -- Same-register ldxdw: reads r2's value as base, writes loaded value back to r2.
  have h4 := ldxdw_same_spec .r2 0x7a81 baseAddr txAmount 4 (by decide) h_amt_lt
  have h5 := mov64_imm_spec .r7 1 (toU64 0x11) 5 (by decide)
  have h6 := ldxdw_spec .r3 .r1 0xa0 (srcState % 256) initR1 srcBalance 6
                        (by decide) h_bal_lt
  have h7 := jlt_reg_spec .r3 .r2 srcBalance txAmount 7 h3dErrPc
  -- Collapse the three cond branches.
  rw [show (if srcState % 256 = toU64 2 then h3dErrPc else 2 + 1) = 3 from by
        have hno : ¬ (srcState % 256 = toU64 2) := by
          show ¬ (srcState % 256 = 2); omega
        rw [if_neg hno]] at h2
  rw [show (if dstState % 256 = toU64 2 then h3dErrPc else 3 + 1) = 4 from by
        have hno : ¬ (dstState % 256 = toU64 2) := by
          show ¬ (dstState % 256 = 2); omega
        rw [if_neg hno]] at h3
  rw [show (if srcBalance < txAmount then h3dErrPc else 7 + 1) = 8 from by
