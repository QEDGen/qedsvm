/-
  Pattern library Layer 3 — the FROZEN-DESTINATION guard's full ENFORCES
  composition on the real p-token binary (sibling of
  `FrozenGuardEnforced.lean`, one `jeq` later).

  From a frozen destination account (`h_branch12 : state = 2` — the
  VIOLATION as hypothesis; the source's `jeq` at 4011 falls through via
  `h_branch11 : state ≠ 2`), the mechanically lifted error path
  (`PTokenTransferDestFrozen_lifted_spec`) runs from the Transfer dispatch
  entry through the taken dest-frozen `jeq` at 4012, the error handler, the
  TokenError logging helper, and the ProgramError encoder to the shared exit
  with `r0 = 17` (TokenError::AccountFrozen), the token cells untouched.
  Composed with the `.exit` into the halting `EnforcedError`: the program
  TERMINATES with `exitCode = some 17 ≠ 0`.
-/

import Generated.PTokenTransferDestFrozenLifted
import Lean
import SVM.Solana.Patterns.Guards

namespace Examples.PTokenDestFrozenGuardEnforced

open SVM.SBPF
open Memory
open Examples.Lifted.PTokenTransferDestFrozen

set_option maxHeartbeats 1600000 in
/-- From a frozen DESTINATION account, the real p-token Transfer HALTS with
    `exitCode = some 17` (AccountFrozen), the token cells untouched — the
    dest-frozen check is ENFORCED, not merely required. -/
theorem p_token_frozen_dest_enforced
    (baseAddr oldMemB_0 vR2Old oldMemD_1 oldMemB_2 oldMemD_3 oldMemB_4 oldMemD_5 vR4Old vR3Old oldMemD_6 oldMemB_7 vR6Old vR7Old oldMemB_8 oldMemB_9 vR5Old vR8Old vR9Old vR10Old oldMemD_10 oldMemD_11 vR0Old : Nat)
    (addr0 : Nat)
    (addr1 : Nat)
    (addr2 : Nat)
    (h_addr0 : addr0 = wrapAdd baseAddr (((wrapAdd oldMemD_5 (toU64 7)) &&& toU64 (-8)) % U64_MODULUS))
    (h_addr1 : addr1 = wrapAdd (toU64 4295072816) ((((let shift := toU64 56 % 64; if (((((((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) < U64_MODULUS / 2 then (((((((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) >>> shift else (let shifted := (((((((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) >>> shift; let highBits := (U64_MODULUS - 1) - (U64_MODULUS / (2 ^ shift) - 1); (shifted ||| highBits) % U64_MODULUS))) <<< (toU64 3 % 64)) % U64_MODULUS))
    (h_addr2 : addr2 = wrapAdd (toU64 4295071984) ((((let shift := toU64 56 % 64; if (((((((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) < U64_MODULUS / 2 then (((((((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) >>> shift else (let shifted := (((((((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) >>> shift; let highBits := (U64_MODULUS - 1) - (U64_MODULUS / (2 ^ shift) - 1); (shifted ||| highBits) % U64_MODULUS))) <<< (toU64 3 % 64)) % U64_MODULUS))
    ...
    (h_branch13 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 12))
    (h_branch14 : toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) ≤ toSigned64 (toU64 5))
    (h_branch15 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 2))
    (h_branch16 : (((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) = toU64 0)
    (h_branch17 : (((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) < toU64 20)
    (h_branch18 : ((((((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) &&& toU64 27) % U64_MODULUS = toU64 26)
    (h_branch19 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 12))
    (h_branch20 : toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) ≤ toSigned64 (toU64 5))
    (h_branch21 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 2))
    (h_branch22 : (((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) = toU64 0)
    (h_branch23 : (((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) ≠ toU64 0)
    (nCuLog12 : Nat)
    (hCuLog12 : ∀ s : State, (step (.call .sol_log_) s).cuConsumed ≤ s.cuConsumed + nCuLog12)
    : SVM.Solana.Patterns.EnforcedError (76 + 1) nCuLog12 198
      ...
      (.r9 ↦ᵣ vR9Old) **
      (.r10 ↦ᵣ vR10Old) **
      (effectiveAddr addr1 0 ↦U64 oldMemD_10) **
      (effectiveAddr addr2 0 ↦U64 oldMemD_11) **
      ...
      (fun rt => ((((((((((((rt.containsRange (effectiveAddr baseAddr 0) 1 = true) ∧
                  ...
                  rt.containsRange (oldMemD_10) (oldMemD_11) = true)
      17 := by
  refine ⟨by decide, ?_⟩
  have h := PTokenTransferDestFrozen_lifted_spec baseAddr oldMemB_0 vR2Old oldMemD_1 oldMemB_2 oldMemD_3 oldMemB_4 oldMemD_5 vR4Old vR3Old oldMemD_6 oldMemB_7 vR6Old vR7Old oldMemB_8 oldMemB_9 vR5Old vR8Old vR9Old vR10Old oldMemD_10 oldMemD_11 vR0Old addr0 addr1 addr2 h_addr0 h_addr1 h_addr2 holdMemD_1_lt holdMemD_3_lt holdMemD_5_lt holdMemD_6_lt holdMemD_10_lt holdMemD_11_lt h_branch0 h_branch1 h_branch2 h_branch3 h_branch4 h_branch5 h_branch6 h_branch7 h_branch8 h_branch9 h_branch10 h_branch11 h_branch12 h_branch13 h_branch14 h_branch15 h_branch16 h_branch17 h_branch18 h_branch19 h_branch20 h_branch21 h_branch22 h_branch23 nCuLog12 hCuLog12
...
#assert_std_axioms_local Examples.Lifted.PTokenTransferDestFrozen.PTokenTransferDestFrozen_lifted_spec
#assert_std_axioms_local Examples.PTokenDestFrozenGuardEnforced.p_token_frozen_dest_enforced

end Examples.PTokenDestFrozenGuardEnforced
