/-
  Pattern library Layer 3 — the balance guard's full ENFORCES composition on
  the REAL p-token binary (`SVM/Solana/Patterns/Guards.lean`,
  `EnforcedError`).

  The mechanically lifted error path
  (`PTokenTransferInsufficient_lifted_spec`, qedlift over the
  insufficient-balance trace) runs from the Transfer dispatch entry (198)
  through the frozen/balance checks, the TAKEN balance `jlt`
  (`h_branch13 : srcBalance < txAmount` — the VIOLATION as hypothesis), the
  error handler, the TokenError logging helper, and the ProgramError encoder
  to the shared exit PC 3542 with `r0 = 1` (InsufficientFunds) and BOTH token
  cells (amount at `addr0+31361`, source balance at `baseAddr+160`) untouched.
  Composing with the `.exit` itself (`cuTripleWithinMem_seq_exit`) yields the
  halting fact: the program TERMINATES with `exitCode = some 1 ≠ 0` — agave
  fails the instruction, the effect never commits.

  The proven happy-path arm REQUIRES the balance check (assumes it); this
  theorem is the ENFORCES direction — together they close the
  requires-without-enforces gap for this check.
-/

import Generated.PTokenTransferInsufficientLifted
import Lean
import SVM.Solana.Patterns.Guards

namespace Examples.PTokenBalanceGuardEnforced

open SVM.SBPF
open Memory
open Examples.Lifted.PTokenTransferInsufficient

set_option maxHeartbeats 1600000 in
/-- From an insufficient-balance pre, the real p-token Transfer HALTS with
    `exitCode = some 1` (InsufficientFunds), the token cells untouched — the
    balance check is ENFORCED, not merely required. -/
theorem p_token_insufficient_balance_enforced
    ...
    : SVM.Solana.Patterns.EnforcedError (80 + 1) nCuLog14 198
      ...
      (fun rt => ((((((((((((((rt.containsRange (effectiveAddr baseAddr 0) 1 = true) ∧
                  ...
                  rt.containsRange (oldMemD_12) (oldMemD_13) = true)
      1 := by
  refine ⟨by decide, ?_⟩
  have h := PTokenTransferInsufficient_lifted_spec baseAddr oldMemB_0 vR2Old
    oldMemD_1 oldMemB_2 oldMemD_3 oldMemB_4 oldMemD_5 vR4Old vR3Old oldMemD_6
    oldMemB_7 vR6Old vR7Old oldMemB_8 oldMemB_9 vR5Old oldMemD_10 oldMemD_11
    vR8Old vR9Old vR10Old oldMemD_12 oldMemD_13 vR0Old addr0 addr1 addr2
    h_addr0 h_addr1 h_addr2 holdMemD_1_lt holdMemD_3_lt holdMemD_5_lt
    holdMemD_6_lt holdMemD_10_lt holdMemD_11_lt holdMemD_12_lt holdMemD_13_lt
    h_branch0 h_branch1 h_branch2 h_branch3 h_branch4 h_branch5 h_branch6
    h_branch7 h_branch8 h_branch9 h_branch10 h_branch11 h_branch12 h_branch13
    h_branch14 h_branch15 h_branch16 h_branch17 h_branch18 h_branch19
    h_branch20 h_branch21 h_branch22 h_branch23 h_branch24 nCuLog14 hCuLog14
...
#assert_std_axioms_local Examples.Lifted.PTokenTransferInsufficient.PTokenTransferInsufficient_lifted_spec
#assert_std_axioms_local Examples.PTokenBalanceGuardEnforced.p_token_insufficient_balance_enforced

end Examples.PTokenBalanceGuardEnforced
