/-
  Pattern library Layer 3 — the FROZEN-ACCOUNT guard's full ENFORCES
  composition on the real p-token binary (sibling of
  `BalanceGuardEnforced.lean`; H3c's check window).

  From a frozen source account (`h_branch11 : state = 2` — the VIOLATION as
  hypothesis), the mechanically lifted error path
  (`PTokenTransferFrozen_lifted_spec`) runs from the Transfer dispatch entry
  through the taken frozen `jeq` at 4011, the error handler, the TokenError
  logging helper, and the ProgramError encoder to the shared exit with
  `r0 = 17` (TokenError::AccountFrozen), the token cells untouched. Composed
  with the `.exit` into the halting `EnforcedError`: the program TERMINATES
  with `exitCode = some 17 ≠ 0`.
-/

import Generated.PTokenTransferFrozenLifted
import Lean
import SVM.Solana.Patterns.Guards

namespace Examples.PTokenFrozenGuardEnforced

open SVM.SBPF
open Memory
open Examples.Lifted.PTokenTransferFrozen

set_option maxHeartbeats 1600000 in
/-- From a frozen source account, the real p-token Transfer HALTS with
    `exitCode = some 17` (AccountFrozen), the token cells untouched — the
    frozen check is ENFORCED, not merely required. -/
theorem p_token_frozen_source_enforced
    ...
    (h_branch12 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 12))
    (h_branch13 : toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) ≤ toSigned64 (toU64 5))
    (h_branch14 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 2))
    (h_branch15 : (((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) = toU64 0)
    (h_branch16 : (((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) < toU64 20)
    (h_branch17 : ((((((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) &&& toU64 27) % U64_MODULUS = toU64 26)
    (h_branch18 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 12))
    (h_branch19 : toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) ≤ toSigned64 (toU64 5))
    (h_branch20 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 2))
    (h_branch21 : (((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) = toU64 0)
    (h_branch22 : (((toU64 17) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) ≠ toU64 0)
    (nCuLog12 : Nat)
    (hCuLog12 : ∀ s : State, (step (.call .sol_log_) s).cuConsumed ≤ s.cuConsumed + nCuLog12)
    : SVM.Solana.Patterns.EnforcedError (75 + 1) nCuLog12 198
...
#assert_std_axioms_local Examples.Lifted.PTokenTransferFrozen.PTokenTransferFrozen_lifted_spec
#assert_std_axioms_local Examples.PTokenFrozenGuardEnforced.p_token_frozen_source_enforced

end Examples.PTokenFrozenGuardEnforced
