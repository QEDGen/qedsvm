/-
  Pattern library Layer 3 — the MINT-MISMATCH guard, limb-2 trace: mints
  agree in dwords 0-1 and differ in the third, diverting at the third `jne`
  (pc 4025). r0 = 3 (TokenError::MintMismatch).
-/

import Generated.PTokenTransferMintMismatchLimb2Lifted
import Lean
import SVM.Solana.Patterns.Guards

namespace Examples.PTokenMintMismatchLimb2GuardEnforced

open SVM.SBPF
open Memory
open Examples.Lifted.PTokenTransferMintMismatchLimb2

set_option maxHeartbeats 1600000 in
/-- From a pre where the mints agree in dwords 0-1 and differ in dword 2,
    the real p-token Transfer HALTS with `exitCode = some 3` (MintMismatch)
    — the mint-equality check is ENFORCED on the limb-2 path. -/
theorem p_token_mint_mismatch_limb2_enforced
    (baseAddr oldMemB_0 vR2Old oldMemD_1 oldMemB_2 oldMemD_3 oldMemB_4 oldMemD_5 vR4Old vR3Old oldMemD_6 oldMemB_7 vR6Old vR7Old oldMemB_8 oldMemB_9 vR5Old oldMemD_10 oldMemD_11 oldMemD_12 oldMemD_13 vR0Old oldMemD_14 oldMemD_15 oldMemD_16 oldMemD_17 vR8Old vR9Old vR10Old oldMemD_18 oldMemD_19 : Nat)
    (addr0 : Nat)
    (addr1 : Nat)
    (addr2 : Nat)
    ...
    (h_branch17 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 12))
    (h_branch18 : toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) ≤ toSigned64 (toU64 5))
    (h_branch19 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 2))
    (h_branch20 : (((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) = toU64 0)
    (h_branch21 : (((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) < toU64 20)
    (h_branch22 : ((((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) &&& toU64 27) % U64_MODULUS = toU64 26)
    ...
    (h_branch27 : (((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) ≠ toU64 0)
    (nCuLog20 : Nat)
    (hCuLog20 : ∀ s : State, (step (.call .sol_log_) s).cuConsumed ≤ s.cuConsumed + nCuLog20)
    : SVM.Solana.Patterns.EnforcedError (90 + 1) nCuLog20 198
      ...
      (.r9 ↦ᵣ vR9Old) **
      (.r10 ↦ᵣ vR10Old) **
      (effectiveAddr addr1 0 ↦U64 oldMemD_18) **
      (effectiveAddr addr2 0 ↦U64 oldMemD_19) ** callStackIs [])
      ((.r0 ↦ᵣ 3) ** callStackIs [] **
      (.r1 ↦ᵣ 0) **
      (effectiveAddr baseAddr 0 ↦ₘ oldMemB_0) **
      (.r2 ↦ᵣ toU64 3) **
      ...
      (.r10 ↦ᵣ vR10Old) **
      (effectiveAddr addr1 0 ↦U64 oldMemD_18) **
      (effectiveAddr addr2 0 ↦U64 oldMemD_19))
...
#assert_std_axioms_local Examples.Lifted.PTokenTransferMintMismatchLimb2.PTokenTransferMintMismatchLimb2_lifted_spec
#assert_std_axioms_local Examples.PTokenMintMismatchLimb2GuardEnforced.p_token_mint_mismatch_limb2_enforced

end Examples.PTokenMintMismatchLimb2GuardEnforced
