/-
  Pattern library Layer 3 — the MINT-MISMATCH guard, limb-1 trace: the
  mints agree in their first dword and differ in the second, so the
  unrolled 4-limb mint compare diverts at its second `jne` (pc 4022).
  Together with the limb-0/2/3 traces this covers every way two 32-byte
  mint pubkeys can first differ. r0 = 3 (TokenError::MintMismatch).
-/

import Generated.PTokenTransferMintMismatchLimb1Lifted
import Lean
import SVM.Solana.Patterns.Guards

namespace Examples.PTokenMintMismatchLimb1GuardEnforced

open SVM.SBPF
open Memory
open Examples.Lifted.PTokenTransferMintMismatchLimb1

set_option maxHeartbeats 1600000 in
/-- From a pre where the mints agree in dword 0 and differ in dword 1, the
    real p-token Transfer HALTS with `exitCode = some 3` (MintMismatch) —
    the mint-equality check is ENFORCED on the limb-1 path. -/
theorem p_token_mint_mismatch_limb1_enforced
    (baseAddr oldMemB_0 vR2Old oldMemD_1 oldMemB_2 oldMemD_3 oldMemB_4 oldMemD_5 vR4Old vR3Old oldMemD_6 oldMemB_7 vR6Old vR7Old oldMemB_8 oldMemB_9 vR5Old oldMemD_10 oldMemD_11 oldMemD_12 oldMemD_13 vR0Old oldMemD_14 oldMemD_15 vR8Old vR9Old vR10Old oldMemD_16 oldMemD_17 : Nat)
    (addr0 : Nat)
    (addr1 : Nat)
    (addr2 : Nat)
    ...
    (h_branch16 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 12))
    (h_branch17 : toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) ≤ toSigned64 (toU64 5))
    (h_branch18 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 2))
    (h_branch19 : (((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) = toU64 0)
    (h_branch20 : (((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) < toU64 20)
    (h_branch21 : ((((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) &&& toU64 27) % U64_MODULUS = toU64 26)
    ...
    (h_branch26 : (((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) ≠ toU64 0)
    (nCuLog18 : Nat)
    (hCuLog18 : ∀ s : State, (step (.call .sol_log_) s).cuConsumed ≤ s.cuConsumed + nCuLog18)
    : SVM.Solana.Patterns.EnforcedError (87 + 1) nCuLog18 198
      ...
      (.r9 ↦ᵣ vR9Old) **
      (.r10 ↦ᵣ vR10Old) **
      (effectiveAddr addr1 0 ↦U64 oldMemD_16) **
      (effectiveAddr addr2 0 ↦U64 oldMemD_17) ** callStackIs [])
      ((.r0 ↦ᵣ 3) ** callStackIs [] **
      (.r1 ↦ᵣ 0) **
      (effectiveAddr baseAddr 0 ↦ₘ oldMemB_0) **
      (.r2 ↦ᵣ toU64 3) **
      ...
      (.r10 ↦ᵣ vR10Old) **
      (effectiveAddr addr1 0 ↦U64 oldMemD_16) **
      (effectiveAddr addr2 0 ↦U64 oldMemD_17))
      ...
      3 := by
  refine ⟨by decide, ?_⟩
  have h := PTokenTransferMintMismatchLimb1_lifted_spec baseAddr oldMemB_0 vR2Old oldMemD_1 oldMemB_2 oldMemD_3 oldMemB_4 oldMemD_5 vR4Old vR3Old oldMemD_6 oldMemB_7 vR6Old vR7Old oldMemB_8 oldMemB_9 vR5Old oldMemD_10 oldMemD_11 oldMemD_12 oldMemD_13 vR0Old oldMemD_14 oldMemD_15 vR8Old vR9Old vR10Old oldMemD_16 oldMemD_17 addr0 addr1 addr2 h_addr0 h_addr1 h_addr2 holdMemD_1_lt holdMemD_3_lt holdMemD_5_lt holdMemD_6_lt holdMemD_10_lt holdMemD_11_lt holdMemD_12_lt holdMemD_13_lt holdMemD_14_lt holdMemD_15_lt holdMemD_16_lt holdMemD_17_lt h_branch0 h_branch1 h_branch2 h_branch3 h_branch4 h_branch5 h_branch6 h_branch7 h_branch8 h_branch9 h_branch10 h_branch11 h_branch12 h_branch13 h_branch14 h_branch15 h_branch16 h_branch17 h_branch18 h_branch19 h_branch20 h_branch21 h_branch22 h_branch23 h_branch24 h_branch25 h_branch26 nCuLog18 hCuLog18
...
#assert_std_axioms_local Examples.Lifted.PTokenTransferMintMismatchLimb1.PTokenTransferMintMismatchLimb1_lifted_spec
#assert_std_axioms_local Examples.PTokenMintMismatchLimb1GuardEnforced.p_token_mint_mismatch_limb1_enforced

end Examples.PTokenMintMismatchLimb1GuardEnforced
