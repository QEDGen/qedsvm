/-
  Pattern library Layer 3 — the MINT-MISMATCH guard on the real p-token
  MintTo arm: the destination token account must belong to the mint being
  minted.

  From a pre where the destination's stored mint differs from the mint
  account's key (the VIOLATION as hypothesis), the lifted error path runs
  to the shared exit with r0 = 3 (TokenError::MintMismatch), cells
  untouched.
-/

import Generated.PTokenMintToMintMismatchLifted
import Lean
import SVM.Solana.Patterns.Guards

namespace Examples.PTokenMintToMintMismatchGuard

open SVM.SBPF
open Memory
open Examples.Lifted.PTokenMintToMintMismatch

set_option maxHeartbeats 1600000 in
/-- From a destination token account of a different mint, the real p-token
    MintTo HALTS with `exitCode = some 3` (TokenError::MintMismatch) — the
    account-mint association check is ENFORCED. -/
theorem p_token_mint_to_mint_mismatch_enforced
    (baseAddr oldMemB_0 vR2Old oldMemD_1 vR0Old oldMemB_2 oldMemB_3 oldMemB_4 oldMemB_5 oldMemB_6 oldMemB_7 oldMemB_8 vR7Old vR10Old oldMemD_9 vR3Old oldMemB_10 oldMemD_11 oldMemD_12 oldMemB_13 oldMemD_14 oldMemD_15 vR4Old oldMemD_16 vR9Old oldMemB_17 vR5Old vR8Old vR6Old oldMemB_18 oldMemB_19 oldMemD_20 oldMemD_21 oldMemD_22 oldMemD_23 : Nat)
    (addr0 : Nat)
    (addr1 : Nat)
    (addr2 : Nat)
    (addr3 : Nat)
    ...
    (h_addr5 : addr5 = wrapAdd (toU64 4295072816) ((((let shift := toU64 56 % 64; if (((((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) < U64_MODULUS / 2 then (((((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) >>> shift else (let shifted := (((((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) >>> shift; let highBits := (U64_MODULUS - 1) - (U64_MODULUS / (2 ^ shift) - 1); (shifted ||| highBits) % U64_MODULUS))) <<< (toU64 3 % 64)) % U64_MODULUS))
    (h_addr6 : addr6 = wrapAdd (toU64 4295071984) ((((let shift := toU64 56 % 64; if (((((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) < U64_MODULUS / 2 then (((((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) >>> shift else (let shifted := (((((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) <<< (toU64 24 % 64)) % U64_MODULUS) >>> shift; let highBits := (U64_MODULUS - 1) - (U64_MODULUS / (2 ^ shift) - 1); (shifted ||| highBits) % U64_MODULUS))) <<< (toU64 3 % 64)) % U64_MODULUS))
    ...
    (h_branch23 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 12))
    (h_branch24 : toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) ≤ toSigned64 (toU64 5))
    (h_branch25 : ¬ toSigned64 ((((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64)) > toSigned64 (toU64 2))
    (h_branch26 : (((toU64 0) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) = toU64 0)
    (h_branch27 : (((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) < toU64 20)
    (h_branch28 : ((((((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) ||| toU64 26) % U64_MODULUS) &&& toU64 27) % U64_MODULUS = toU64 26)
    ...
    (h_branch33 : (((toU64 3) <<< (toU64 32 % 64)) % U64_MODULUS) >>> (toU64 32 % 64) ≠ toU64 0)
    ...
    (nCuLog24 : Nat)
    (hCuLog24 : ∀ s : State, (step (.call .sol_log_) s).cuConsumed ≤ s.cuConsumed + nCuLog24)
    : SVM.Solana.Patterns.EnforcedError (109 + 1) nCuLog24 198
      ...
      (effectiveAddr addr5 0 ↦U64 oldMemD_22) **
      (effectiveAddr addr6 0 ↦U64 oldMemD_23) ** callStackIs [])
      ((.r0 ↦ᵣ 3) ** callStackIs [] **
      (.r1 ↦ᵣ 0) **
      (effectiveAddr baseAddr 0 ↦ₘ oldMemB_0) **
      (.r2 ↦ᵣ toU64 3) **
      ...
      (fun rt => (((((((((((((((((((((rt.containsRange (effectiveAddr baseAddr 0) 1 = true) ∧
                  ...
                  rt.containsRange (oldMemD_22) (oldMemD_23) = true)
      3 := by
  refine ⟨by decide, ?_⟩
  have h := PTokenMintToMintMismatch_lifted_spec baseAddr oldMemB_0 vR2Old oldMemD_1 vR0Old oldMemB_2 oldMemB_3 oldMemB_4 oldMemB_5 oldMemB_6 oldMemB_7 oldMemB_8 vR7Old vR10Old oldMemD_9 vR3Old oldMemB_10 oldMemD_11 oldMemD_12 oldMemB_13 oldMemD_14 oldMemD_15 vR4Old oldMemD_16 vR9Old oldMemB_17 vR5Old vR8Old vR6Old oldMemB_18 oldMemB_19 oldMemD_20 oldMemD_21 oldMemD_22 oldMemD_23 addr0 addr1 addr2 addr3 addr4 addr5 addr6 h_addr0 h_addr1 h_addr2 h_addr3 h_addr4 h_addr5 h_addr6 holdMemD_1_lt holdMemB_0_lt holdMemB_2_lt holdMemB_3_lt holdMemB_4_lt holdMemB_5_lt holdMemB_6_lt holdMemB_7_lt holdMemB_8_lt holdMemD_12_lt holdMemD_15_lt holdMemD_16_lt holdMemD_20_lt holdMemD_21_lt holdMemD_22_lt holdMemD_23_lt h_branch0 h_branch1 h_branch2 h_branch3 h_branch4 h_branch5 h_branch6 h_branch7 h_branch8 h_branch9 h_branch10 h_branch11 h_branch12 h_branch13 h_branch14 h_branch15 h_branch16 h_branch17 h_branch18 h_branch19 h_branch20 h_branch21 h_branch22 h_branch23 h_branch24 h_branch25 h_branch26 h_branch27 h_branch28 h_branch29 h_branch30 h_branch31 h_branch32 h_branch33 hReloadLt_3658 h_alias_3658 nCuLog24 hCuLog24
...
#assert_std_axioms_local Examples.Lifted.PTokenMintToMintMismatch.PTokenMintToMintMismatch_lifted_spec
#assert_std_axioms_local Examples.PTokenMintToMintMismatchGuard.p_token_mint_to_mint_mismatch_enforced

end Examples.PTokenMintToMintMismatchGuard
