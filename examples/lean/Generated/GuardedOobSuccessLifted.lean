...
namespace Examples.Lifted.GuardedOobSuccess

open SVM.SBPF

/-- `.text` bytes extracted from the .so by qedlift. -/
def GuardedOobSuccessBytes : ByteArray := ⟨#[
...
def GuardedOobSuccessTextOffset : Nat := 0x100000120

/-- Decoded form of the .text bytes. -/
def GuardedOobSuccessInsns : Array Insn := #[
  .ldx .dword .r2 .r1 0,
  .jeq .r2 (.imm (0)) 7,
  .ldx .dword .r3 .r1 8,
  .add64 .r3 (.reg .r2),
...
def GuardedOobSuccessFnRegistry : List (Nat × Nat) := [(1910755201, 0)]
...
open Memory in
theorem GuardedOobSuccess_lifted_spec
...
open Memory in
theorem GuardedOobSuccess_balance_correct
    (baseAddr oldMemD_0 vR2Old oldMemD_1 vR3Old vR0Old : Nat)
    (holdMemD_0_lt : oldMemD_0 < 2 ^ 64)
    (holdMemD_1_lt : oldMemD_1 < 2 ^ 64)
    (h_branch0 : oldMemD_0 ≠ toU64 0)
    ...
    : cuTripleWithinMem 7 0 0 10
      ...
      (fun rt => ((rt.containsRange (effectiveAddr baseAddr 0) 8 = true) ∧
                  rt.containsRange (effectiveAddr baseAddr 8) 8 = true) ∧
                  rt.containsWritable (effectiveAddr baseAddr 8) 8 = true) := by
  have h := GuardedOobSuccess_lifted_spec baseAddr oldMemD_0 vR2Old oldMemD_1 vR3Old vR0Old holdMemD_0_lt holdMemD_1_lt h_branch0
...
One PATH of the account GuardedOob (handler credit) transition: composed from the
running triple (`GuardedOobSuccess_balance_correct`) and the shared `.exit` via
...
theorem GuardedOobSuccess_transition_path
    (baseAddr oldMemD_0 vR2Old oldMemD_1 vR3Old vR0Old : Nat)
    (holdMemD_0_lt : oldMemD_0 < 2 ^ 64)
    (holdMemD_1_lt : oldMemD_1 < 2 ^ 64)
    (h_branch0 : oldMemD_0 ≠ toU64 0)
  ...
  · have framed := cuTripleWithinMem_frame_right
      ( callStackIs [] )
      (by sl_pcfree) (GuardedOobSuccess_balance_correct baseAddr oldMemD_0 vR2Old oldMemD_1 vR3Old vR0Old holdMemD_0_lt holdMemD_1_lt h_branch0 h_noovf0)
    sl_exact framed
end Examples.Lifted.GuardedOobSuccess
