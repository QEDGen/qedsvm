/-
  Generated by `qedlift` from `tests/fixtures/counter_with_helper.so`.
...
namespace Examples.Lifted.CounterWithHelper

open SVM.SBPF

/-- `.text` bytes extracted from the .so by qedlift. -/
def CounterWithHelperBytes : ByteArray := ⟨#[
...
def CounterWithHelperTextOffset : Nat := 0x100000120
...
def CounterWithHelperFnRegistry : List (Nat × Nat) := [(1669671676, 0), (1910755201, 4)]
...
open Memory in
theorem CounterWithHelper_lifted_spec
...
open Memory in
theorem CounterWithHelper_balance_correct
    (baseAddr oldMemD_0 vR2Old vR6Old vR7Old vR8Old vR9Old vR10Old oldMemD_1 vR3Old vR0Old : Nat)
    (addr0 : Nat)
    (h_addr0 : addr0 = wrapAdd baseAddr (toU64 8))
    (holdMemD_0_lt : oldMemD_0 < 2 ^ 64)
    ...
    : cuTripleWithinMem 8 0 4 8
      ...
      (fun rt => ((rt.containsRange (effectiveAddr baseAddr 0) 8 = true) ∧
                  rt.containsRange (effectiveAddr addr0 0) 8 = true) ∧
                  rt.containsWritable (effectiveAddr addr0 0) 8 = true) := by
  have h := CounterWithHelper_lifted_spec baseAddr oldMemD_0 vR2Old vR6Old vR7Old vR8Old vR9Old vR10Old oldMemD_1 vR3Old vR0Old addr0 h_addr0 holdMemD_0_lt holdMemD_1_lt
  rw [← wrapAdd_of_lt h_noovf0]
  exact h

end Examples.Lifted.CounterWithHelper
