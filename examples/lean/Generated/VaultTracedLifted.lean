...
namespace Examples.Lifted.Vault

open SVM.SBPF

/-- `.text` bytes extracted from the .so by qedlift. -/
def VaultBytes : ByteArray := ⟨#[
...
def VaultTextOffset : Nat := 0x100000120

/-- Decoded form of the .text bytes. -/
def VaultInsns : Array Insn := #[
  .ldx .dword .r2 .r1 32,
  .add64 .r2 (.imm (1)),
  .stx .dword .r1 32 .r2,
  .mov64 .r0 (.imm (0)),
...
theorem Vault_balance_correct
    ...
    : cuTripleWithinMem 4 0 0 4
      ...
      (fun rt => (rt.containsRange (effectiveAddr baseAddr 32) 8 = true) ∧
                  rt.containsWritable (effectiveAddr baseAddr 32) 8 = true) := by
  have h := Vault_lifted_spec baseAddr oldMemD_0 vR2Old vR0Old holdMemD_0_lt
  rw [← wrapAdd_one_of_lt h_noovf0]
  exact h

end Examples.Lifted.Vault
