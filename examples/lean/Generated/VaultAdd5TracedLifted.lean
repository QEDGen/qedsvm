/-
  Generated by `qedlift` from `qedsvm-rs/tests/fixtures/vault_add5.so`.
...
`SVM/SBPF/Macros.lean` (`vault_add5_macro_spec_auto`) where the
...
namespace Examples.Lifted.VaultAdd5

open SVM.SBPF

/-- `.text` bytes extracted from the .so by qedlift. -/
def VaultAdd5Bytes : ByteArray := ⟨#[
  0x79, 0x12, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 
  0x07, 0x02, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 
...
def VaultAdd5TextOffset : Nat := 0x100000120

/-- Decoded form of the .text bytes. -/
def VaultAdd5Insns : Array Insn := #[
  .ldx .dword .r2 .r1 32,
  .add64 .r2 (.imm (5)),
  .stx .dword .r1 32 .r2,
  .mov64 .r0 (.imm (0)),
...
def VaultAdd5FnRegistry : List (Nat × Nat) := [(1910755201, 0)]

/-- The bytes decode exactly to the expected instruction array. -/
theorem VaultAdd5_decodes :
    Decode.decodeProgram VaultAdd5Bytes VaultAdd5FnRegistry = some VaultAdd5Insns := by
  native_decide
...
theorem VaultAdd5_lifted_spec
    ...
    : cuTripleWithinMem 4 0 0 4
      (((((CodeReq.singleton 0 (.ldx .dword .r2 .r1 32)).union
        (CodeReq.singleton 1 (.add64 .r2 (.imm (5))))).union
      ...
      (.r0 ↦ᵣ vR0Old))
      ((.r1 ↦ᵣ baseAddr) **
      (effectiveAddr baseAddr 32 ↦U64 wrapAdd oldMemD_0 (toU64 5)) **
...
open Memory in
theorem VaultAdd5_balance_correct
    (baseAddr oldMemD_0 vR2Old vR0Old : Nat)
    (holdMemD_0_lt : oldMemD_0 < 2 ^ 64)
    (h_noovf0 : oldMemD_0 + 5 < 2 ^ 64)
    : cuTripleWithinMem 4 0 0 4
      (((((CodeReq.singleton 0 (.ldx .dword .r2 .r1 32)).union
        (CodeReq.singleton 1 (.add64 .r2 (.imm (5))))).union
        (CodeReq.singleton 2 (.stx .dword .r1 32 .r2))).union
      ...
      (fun rt => (rt.containsRange (effectiveAddr baseAddr 32) 8 = true) ∧
                  rt.containsWritable (effectiveAddr baseAddr 32) 8 = true) := by
  have h := VaultAdd5_lifted_spec baseAddr oldMemD_0 vR2Old vR0Old holdMemD_0_lt
  rw [← wrapAdd_const_of_lt h_noovf0]
  exact h

end Examples.Lifted.VaultAdd5
