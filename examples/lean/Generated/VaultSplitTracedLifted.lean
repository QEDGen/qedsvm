/-
  Generated by `qedlift` from `tests/fixtures/vault_split.so`.
...
`SVM/SBPF/Macros.lean` (`vault_split_macro_spec_auto`) where the
...
namespace Examples.Lifted.VaultSplit

open SVM.SBPF

/-- `.text` bytes extracted from the .so by qedlift. -/
def VaultSplitBytes : ByteArray := ⟨#[
...
def VaultSplitTextOffset : Nat := 0x100000120

/-- Decoded form of the .text bytes. -/
def VaultSplitInsns : Array Insn := #[
  .ldx .dword .r2 .r1 32,
  .add64 .r2 (.imm (1)),
  .stx .dword .r1 32 .r2,
  .ldx .byte .r0 .r1 5,
...
def VaultSplitFnRegistry : List (Nat × Nat) := [(1910755201, 0)]
...
open Memory in
theorem VaultSplit_lifted_spec
    (baseAddr oldMemD_0 vR2Old oldMemB_1 vR0Old : Nat)
    ...
    : cuTripleWithinMem 4 0 0 4
      (((((CodeReq.singleton 0 (.ldx .dword .r2 .r1 32)).union
        ...
        (CodeReq.singleton 3 (.ldx .byte .r0 .r1 5))))
      ((.r1 ↦ᵣ baseAddr) **
      ...
      (fun rt => ((rt.containsRange (effectiveAddr baseAddr 32) 8 = true) ∧
                  rt.containsWritable (effectiveAddr baseAddr 32) 8 = true) ∧
                  rt.containsRange (effectiveAddr baseAddr 5) 1 = true) := by
...
example : ∃ s,
    ((.r1 ↦ᵣ 17179869184) **
      ...
      (.r2 ↦ᵣ 0) **
      (effectiveAddr 17179869184 5 ↦ₘ 0) **
      (.r0 ↦ᵣ 0)) s := by
...
open Memory in
theorem VaultSplit_balance_correct
    (baseAddr oldMemD_0 vR2Old oldMemB_1 vR0Old : Nat)
    (holdMemD_0_lt : oldMemD_0 < 2 ^ 64)
    ...
    : cuTripleWithinMem 4 0 0 4
      ...
      (fun rt => ((rt.containsRange (effectiveAddr baseAddr 32) 8 = true) ∧
                  rt.containsWritable (effectiveAddr baseAddr 32) 8 = true) ∧
                  rt.containsRange (effectiveAddr baseAddr 5) 1 = true) := by
  have h := VaultSplit_lifted_spec baseAddr oldMemD_0 vR2Old oldMemB_1 vR0Old holdMemD_0_lt
  rw [← wrapAdd_one_of_lt h_noovf0]
  exact h

end Examples.Lifted.VaultSplit
