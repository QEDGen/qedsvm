...
namespace Examples.Lifted.VaultDescriptor

open SVM.SBPF

/-- `.text` bytes extracted from the .so by qedlift. -/
def VaultDescriptorBytes : ByteArray := ⟨#[
...
def VaultDescriptorTextOffset : Nat := 0x100000120

/-- Decoded form of the .text bytes. -/
def VaultDescriptorInsns : Array Insn := #[
  .ldx .dword .r2 .r1 32,
  .add64 .r2 (.imm (1)),
  .stx .dword .r1 32 .r2,
  .mov64 .r0 (.imm (0)),
...
def VaultDescriptorFnRegistry : List (Nat × Nat) := [(1910755201, 0)]

/-- The bytes decode exactly to the expected instruction array. -/
theorem VaultDescriptor_decodes :
    Decode.decodeProgram VaultDescriptorBytes VaultDescriptorFnRegistry = some VaultDescriptorInsns := by
  native_decide
...
open Memory in
theorem VaultDescriptor_lifted_spec
...
open Memory in
theorem VaultDescriptor_balance_correct
    (baseAddr oldMemD_0 vR2Old vR0Old : Nat)
    (holdMemD_0_lt : oldMemD_0 < 2 ^ 64)
    ...
    : cuTripleWithinMem 4 0 0 4
      ...
      (fun rt => (rt.containsRange (effectiveAddr baseAddr 32) 8 = true) ∧
                  rt.containsWritable (effectiveAddr baseAddr 32) 8 = true) := by
  have h := VaultDescriptor_lifted_spec baseAddr oldMemD_0 vR2Old vR0Old holdMemD_0_lt
  rw [← wrapAdd_one_of_lt h_noovf0]
  exact h

end Examples.Lifted.VaultDescriptor
