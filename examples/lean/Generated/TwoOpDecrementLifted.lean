...
namespace Examples.Lifted.TwoOpDecrement

open SVM.SBPF

/-- `.text` bytes extracted from the .so by qedlift. -/
def TwoOpDecrementBytes : ByteArray := ⟨#[
...
def TwoOpDecrementTextOffset : Nat := 0x100000120

/-- Decoded form of the .text bytes. -/
def TwoOpDecrementInsns : Array Insn := #[
  .ldx .dword .r2 .r1 1,
  .ldx .byte .r3 .r1 0,
  .jeq .r3 (.imm (1)) 6,
  .mov64 .r0 (.imm (1)),
...
def TwoOpDecrementFnRegistry : List (Nat × Nat) := [(1910755201, 0)]

/-- The bytes decode exactly to the expected instruction array. -/
theorem TwoOpDecrement_decodes :
    Decode.decodeProgram TwoOpDecrementBytes TwoOpDecrementFnRegistry = some TwoOpDecrementInsns := by
  native_decide
...
theorem TwoOpDecrement_lifted_spec
    ...
    : cuTripleWithinMem 9 0 0 9
      ((((((((((CodeReq.singleton 0 (.ldx .dword .r2 .r1 1)).union
        ...
        (CodeReq.singleton 3 (.mov64 .r0 (.imm (1))))).union
        (CodeReq.singleton 4 (.jne .r3 (.imm (2)) 9))).union
        (CodeReq.singleton 5 (.mov64 .r3 (.imm (-1))))).union
      ...
      (.r0 ↦ᵣ vR0Old))
      ((.r1 ↦ᵣ baseAddr) **
      (effectiveAddr baseAddr 1 ↦U64 wrapAdd oldMemD_0 (toU64 (-1))) **
