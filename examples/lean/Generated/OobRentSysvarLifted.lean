/-
  Generated by `qedlift` from `tests/fixtures/oob_rent_sysvar.so`.
...
`SVM/SBPF/Macros.lean` (`oob_rent_sysvar_macro_spec_auto`) where the
...
namespace Examples.Lifted.OobRentSysvar

open SVM.SBPF

/-- `.text` bytes extracted from the .so by qedlift. -/
def OobRentSysvarBytes : ByteArray := ⟨#[
  0x07, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 
  0x85, 0x10, 0x00, 0x00, 0xf6, 0x88, 0x71, 0xbf, 
  0xb7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
  0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
]⟩

/-- Text section file-offset: 0x100000120. -/
def OobRentSysvarTextOffset : Nat := 0x100000120

/-- Decoded form of the .text bytes. -/
def OobRentSysvarInsns : Array Insn := #[
...
def OobRentSysvarFnRegistry : List (Nat × Nat) := [(1910755201, 0)]
...
open Memory in
theorem OobRentSysvar_lifted_spec
    (baseAddr : Nat)
...
open Memory in
theorem OobRentSysvar_fault_correct
    (baseAddr : Nat)
    (nCuOob : Nat)
    (hCuOob : ∀ s : State,
        (step (.call .sol_get_rent_sysvar) s).cuConsumed ≤ s.cuConsumed + nCuOob)
    : cuTripleFaultsWithinMem (1 + 1) (0 + nCuOob) 0
      (((CodeReq.singleton 0 (.add64 .r1 (.imm (268435456))))).union
        (CodeReq.singleton 1 (.call .sol_get_rent_sysvar)))
      ((.r1 ↦ᵣ baseAddr))
      (fun rt => (True) ∧ rt.containsWritable (wrapAdd baseAddr (toU64 268435456)) 17 = false)
      .accessViolation := by
  refine cuTripleWithinMem_seq_fault ?_ (OobRentSysvar_lifted_spec baseAddr) (call_sol_get_rent_sysvar_faults_oob_spec (wrapAdd baseAddr (toU64 268435456)) 1 nCuOob hCuOob)
  repeat' apply CodeReq.Disjoint_union_left
  all_goals exact CodeReq.singleton_disjoint_singleton _ _ (by decide)

end Examples.Lifted.OobRentSysvar
