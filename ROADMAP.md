...
1. `PTokenTransferArmThirdCalleeSpec.lean` — pinocchio account-validation callee at byte 0x9a88 (~30-60 insns; ~2h; **highest risk** — first non-compiler-rt callee, pattern unfamiliar).
2. `PTokenTransferArmThirdCall.lean` — 5-insn glue for the third `call_local` (~1.5h).
