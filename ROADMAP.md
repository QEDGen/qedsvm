...
**Headline numbers**: 142 instruction-spec Hoare triples in `InstructionSpecs.lean` (full ALU + branch + memory + call/return + 24+ syscalls); ~59 Rust tests green (26 diff-mollusk incl. **p-token Transfer at 76 CU byte+CU-identical to mollusk** + 5 precompile + 11 svm_api + 17 other); end-to-end Hoare-triple witness for a 4-instruction hand-encoded program (`examples/lean/ByteIncrement.lean` — Layer 3a per the README); **Hoare triple over 8 instructions of compiler-emitted p-token bytecode** with 4 branch composition (`examples/lean/PTokenValidationPrelude.lean` — Layer 3b per the README, first artifact).

**Next milestone — Layer 3b scaled**: extend the validation prelude proof to the full p-token Transfer happy path (~250-400 instructions, ~20-30 branches, chaining through `call_local`/`exit_pops`). Substrate is in place — primitives, bytes→spec bridge, multi-branch composition via the magic-match hypothesis collapse all proven. Remaining work is bounded but multi-week. See [`docs/p-token-spike.md`](docs/p-token-spike.md) for the methodology validation that preceded the first artifact.
